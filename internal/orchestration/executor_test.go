...
func TestExecutorOnEventUnsubscribeStaysCorrectAfterEarlierRemoval(t *testing.T) {
	e := NewExecutor(OrchestratorConfig{MaxConcurrentAgents: 1})

	var first atomic.Int32
	var second atomic.Int32

	unsubFirst := e.OnEvent(func(ExecutorEvent) { first.Add(1) })
	unsubSecond := e.OnEvent(func(ExecutorEvent) { second.Add(1) })
	unsubFirst()
	unsubSecond()
	...
	time.Sleep(20 * time.Millisecond)
	if first.Load() != 0 {
		t.Fatalf("expected first handler removed, got %d", first.Load())
	}
	if second.Load() != 0 {
		t.Fatalf("expected second handler removed after earlier unsubscribe, got %d", second.Load())
	}
}
...
func TestExecutor_LocalWorktreeTurnGuard(t *testing.T) {
	e := NewExecutor(OrchestratorConfig{MaxConcurrentAgents: 10})

	wt := func(id string) TaskDefinition {
		return TaskDefinition{ID: id, Title: id, RequiredSkills: []string{"general"}, ExecutionMode: "local_worktree"}
	}

	_ = e.Submit(wt("wt1"))
	_ = e.Submit(wt("wt2"))

	ok1, err := e.Checkout("wt1", "alice")
	if err != nil || !ok1 {
		t.Fatalf("first local_worktree checkout should succeed: ok=%v err=%v", ok1, err)
	}

	ok2, err := e.Checkout("wt2", "bob")
	if err != nil {
		t.Fatalf("unexpected error on second checkout: %v", err)
	}
	if ok2 {
		t.Fatal("second local_worktree checkout must be blocked while one is active")
	}

	res := ""
	if err := e.Release("wt1", &res, nil); err != nil {
		t.Fatalf("release failed: %v", err)
	}

	ok3, err := e.Checkout("wt2", "bob")
	if err != nil || !ok3 {
		t.Fatalf("local_worktree checkout should succeed after release: ok=%v err=%v", ok3, err)
	}
}
