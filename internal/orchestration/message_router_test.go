...
func TestExplicitMentionRoutes(t *testing.T) {
	...
	withPunctuation := mr.Route("@fe, can you review this?", agents)
	if withPunctuation.Primary != "fe" {
		t.Errorf("expected primary='fe' for punctuated @mention, got '%s'", withPunctuation.Primary)
	}
}
