...
func TestResolveCodexCLIPathUsesCodexExecutable(t *testing.T) {
	path, name, err := resolveCodexCLIPath(func(file string) (string, error) {
		switch file {
		case "codex":
	...
	if err != nil {
		t.Fatalf("resolveCodexCLIPath: %v", err)
	}
	if name != "codex" {
		t.Fatalf("expected codex executable, got %q", name)
	}
	if path != "/usr/local/bin/codex" {
		t.Fatalf("expected codex path, got %q", path)
	}
}

func TestResolveCodexCLIPathFallsBackToCodexLB(t *testing.T) {
	path, name, err := resolveCodexCLIPath(func(file string) (string, error) {
		switch file {
		case "codex-lb":
			return "/usr/local/bin/codex-lb", nil
		default:
			return "", exec.ErrNotFound
		}
	})
	...
	if name != "codex-lb" {
		t.Fatalf("expected codex-lb executable, got %q", name)
	}
	if path != "/usr/local/bin/codex-lb" {
		t.Fatalf("expected codex-lb path, got %q", path)
	}
}

func TestResolveCodexCLIPathAcceptsWindowsShim(t *testing.T) {
	path, name, err := resolveCodexCLIPath(func(file string) (string, error) {
		switch file {
		case "codex.cmd":
	...
	if name != "codex.cmd" {
		t.Fatalf("expected codex.cmd executable, got %q", name)
	}
	if path != `C:\Tools\codex.cmd` {
		t.Fatalf("expected codex.cmd path, got %q", path)
	}
}
