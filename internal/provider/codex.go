...
func CreateCodexCLIStreamFn(agentSlug string) agent.StreamFn {
	return func(msgs []agent.Message, tools []agent.AgentTool) <-chan agent.StreamChunk {
		...
		go func() {
			...
			_, binaryName, err := resolveCodexCLIPath(codexLookPath)
			if err != nil {
			...
			text, err := runCodexOnce(binaryName, systemPrompt, prompt, cwd, func(event CodexStreamEvent) {
...
func RunCodexOneShot(systemPrompt, prompt, cwd string) (string, error) {
	...
	return runCodexOnce(binaryName, systemPrompt, prompt, cwd, nil)
}

func runCodexOnce(binaryName, systemPrompt, prompt, cwd string, onEvent func(CodexStreamEvent)) (string, error) {
	args := buildCodexArgs(cwd, config.ResolveCodexModel(cwd))
	cmd := codexCommand(binaryName, args...)
	cmd.Dir = cwd
