...
// updateTextInput handles keystrokes during text-entry phases.
...
func MemoryOptions() []PickerOption {
	return []PickerOption{
		{
			Label:       "No shared memory (default)",
			Value:       config.MemoryBackendNone,
			Description: "Local-only baseline. Durable state stays in broker persistence and configured cloud backup.",
		},
	}
}
...
func (f InitFlowModel) readinessChecks() []initReadinessCheck {
	...
	providerName := strings.TrimSpace(f.provider)
	if providerName == "" {
	...
	checks := []initReadinessCheck{
		...
		{
			Label:  "LLM runtime",
			Status: providerRuntimeStatus(providerName),
			Detail: providerRuntimeDetail(providerName),
		},
		{
...
func apiKeyReadinessDetail(ok bool) string {
	if ok {
		return "Workspace API key is configured."
	}
	return "Paste your workspace API key to enable provider-backed actions."
}
...
func providerRuntimeStatus(providerName string) string {
...
func providerRuntimeDetail(providerName string) string {
	switch strings.TrimSpace(providerName) {
	case "", "claude-code":
...
func (f InitFlowModel) phaseText() (heading, instructions string) {
	...
	case InitAPIKey:
		return "Enter Workspace API Key", "Paste your workspace API key if you use provider-backed actions or integrations."
	case InitProviderChoice:
