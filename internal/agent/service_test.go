...
func TestSubscribeUnsubscribeKeepsRemovalStableAfterEarlierUnsubscribes(t *testing.T) {
	svc := newTestService(t, nil)

	var first atomic.Int32
	var second atomic.Int32

	unsubFirst := svc.Subscribe(func() { first.Add(1) })
	unsubSecond := svc.Subscribe(func() { second.Add(1) })
	unsubFirst()
	unsubSecond()

	if _, err := svc.Create(AgentConfig{Slug: "listener-order", Name: "Listener Order"}); err != nil {
		t.Fatalf("Create: %v", err)
	}
	if first.Load() != 0 {
		t.Fatalf("expected first listener removed, got %d", first.Load())
	}
	if second.Load() != 0 {
		t.Fatalf("expected second listener removed after earlier unsubscribe, got %d", second.Load())
	}
}

func TestCreateDoesNotDeadlockWhenListenerReentersService(t *testing.T) {
	svc := newTestService(t, nil)
	...
	go func() {
		_, err := svc.Create(AgentConfig{Slug: "reentrant", Name: "Reentrant"})
		errCh <- err
	}()
	...
	case <-time.After(time.Second):
		t.Fatal("create deadlocked under re-entrant listener")
	}

	select {
	case <-done:
	case <-time.After(time.Second):
		t.Fatal("listener did not complete")
	}
}
