...
func TestShredPreservesWorkspaceState(t *testing.T) {
	dir := withRuntimeHome(t)
	paths := seedWorkspace(t, dir)
	...
	if len(res.Removed) != 0 {
		t.Fatalf("expected no destructive removals, got %v", res.Removed)
	}

	// Shred used to wipe these files. It is now intentionally non-destructive:
	// local office state is user data, not disposable runtime.
	for _, label := range []string{
