...
type TeamRequestArgs struct {
	Kind                string                 `json:"kind,omitempty" jsonschema:"One of: choice, confirm, freeform, approval, secret, epistemic_check. Defaults to choice."`
	Channel             string                 `json:"channel,omitempty" jsonschema:"Channel slug. Defaults to the agent's current channel or general."`
...
func handleTeamRuntimeState(ctx context.Context, _ *mcp.CallToolRequest, args TeamRuntimeStateArgs) (*mcp.CallToolResult, any, error) {
	...
	snapshot := team.BuildRuntimeSnapshot(team.RuntimeSnapshotInput{
		...
		Requests:      requests,
		Recent:        recent,
		Observability: fetchSessionObservability(ctx),
...
func summarizeTaskRuntime(channel string, tasks []brokerTaskSummary) string {
	...
	for _, task := range tasks {
		...
		if path := strings.TrimSpace(preferredWorkspacePath(task.WorkspacePath, task.WorktreePath)); path != "" {
			line += " · working_directory " + path
		}
		isolatedTasks = append(isolatedTasks, line)
	}
	if len(isolatedTasks) > 0 {
		lines = append(lines, "", "Isolated task workspaces:")
		lines = append(lines, isolatedTasks...)
		lines = append(lines, "", "For isolated tasks, use the listed working_directory for local file and bash tools.")
	}
...
func preferredWorkspacePath(workspacePath, worktreePath string) string {
	if trimmed := strings.TrimSpace(workspacePath); trimmed != "" {
		return trimmed
	}
	return strings.TrimSpace(worktreePath)
}
...
func humanRequestOptionDefaults(kind string) ([]HumanInterviewOption, string) {
	...
	case "epistemic_check":
		return []HumanInterviewOption{
			...
			{ID: "safe_inference", Label: "Safe inference", Description: "Proceed, but keep the next step narrow and explicit about what remains inference.", RequiresText: true, TextHint: "Type the accepted inference and the narrow next safe step."},
			{ID: "needs_more_evidence", Label: "Need more evidence", Description: "Do not proceed yet; gather the missing evidence first.", RequiresText: true, TextHint: "Type the missing evidence, command, file, or external check required next."},
			{ID: "do_not_proceed", Label: "Do not proceed", Description: "The uncertainty is too high or the risk is unacceptable.", RequiresText: true, TextHint: "Type why this should stop here and what safer fallback should happen instead."},
		}, "facts_verified"
...
func fetchSessionObservability(ctx context.Context) team.SessionObservabilitySnapshot {
	var snapshot team.SessionObservabilitySnapshot
	if err := brokerGetJSON(ctx, "/sessions", &snapshot); err != nil {
