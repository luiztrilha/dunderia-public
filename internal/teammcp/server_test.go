...
func initUsableGitWorktree(t *testing.T, dir string) {
	t.Helper()
...
func TestHandleTeamChannelCreateRequiresExplicitSlug(t *testing.T) {
	...
	gotSlugs := make([]string, 0, len(channelsResult.Channels))
	for _, channel := range channelsResult.Channels {
		gotSlugs = append(gotSlugs, channel.Slug)
	}
	if slices.Contains(gotSlugs, "launch") {
		t.Fatalf("expected omitted-slug create to leave baseline channels unchanged, got %+v", gotSlugs)
	}
	if !slices.Contains(gotSlugs, "general") {
		t.Fatalf("expected baseline channels to include general, got %+v", gotSlugs)
	}
}
...
func TestHandleTeamMemoryQueryAutoKeepsPrivateHitsWhenSharedMemoryIsDisabled(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	...
	text := textFromResult(t, result)
	if askedQuery != "" {
		t.Fatal("shared query should stay disabled in this fork")
	}
	if !strings.Contains(text, "Private memory:") || !strings.Contains(text, "launch-brief") {
		t.Fatalf("expected private memory hits, got %q", text)
	}
	if strings.Contains(text, "Shared memory:") || strings.Contains(text, "Shared launch history from Nex.") {
		t.Fatalf("did not expect shared memory hits in this fork, got %q", text)
	}
}

func TestHandleTeamMemoryPromoteExplainsSharedMemoryIsDisabled(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("WUPHF_MEMORY_BACKEND", "nex")
	t.Setenv("WUPHF_API_KEY", "nex-test-key")
	t.Setenv("WUPHF_NO_NEX", "")
	...
	text := textFromResult(t, result)
	if !strings.Contains(text, "shared external memory is not active for this run") {
		t.Fatalf("expected disabled shared-memory explanation, got %q", text)
	}
	if postedBody != "" {
		t.Fatalf("expected no shared-memory write, got %q", postedBody)
	}
}

func TestHandleTeamMemoryQuerySharedReportsNoHitsWhenSharedMemoryIsDisabled(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("WUPHF_MEMORY_BACKEND", "nex")
	t.Setenv("WUPHF_API_KEY", "nex-test-key")
	t.Setenv("WUPHF_NO_NEX", "")
	...
	text := textFromResult(t, result)
	if !strings.Contains(text, "Shared memory: no relevant hits.") {
		t.Fatalf("expected disabled shared-memory fallback, got %q", text)
	}
	if strings.Contains(text, "Routing hints:") {
		t.Fatalf("did not expect routing hints without shared hits, got %q", text)
	}
}
...
func TestHandleTeamTaskStatusReportsWorktreeIsolation(t *testing.T) {
	...
	if !strings.Contains(normalizedText, ".wuphf/task-worktrees/") {
	...
	normalizedTasksText := filepath.ToSlash(tasksText)
	...
	if !strings.Contains(tasksText, "working_directory ") || !strings.Contains(normalizedTasksText, ".wuphf/task-worktrees/") {
...
func TestHandleTeamTaskReturnsWorktreeGuidance(t *testing.T) {
	...
	result, _, err := handleTeamTask(ctx, nil, TeamTaskArgs{
		...
		ID:      created.Task.ID,
		MySlug:  "fe",
		Details: teamMCPTestStructuredHandoff("review-ready", "Reviewed worktree task.", "No downstream action required."),
	})
	if err != nil {
	...
	if !strings.Contains(text, "working_directory ") || !strings.Contains(normalizedText, ".wuphf/task-worktrees/") {
		t.Fatalf("expected working_directory guidance in %q", text)
	}
}
...
func TestHandleTeamRuntimeStateIncludesRecoveryAndCapabilities(t *testing.T) {
	...
	b.SetSessionObservabilityFn(func() team.SessionObservabilitySnapshot {
		return team.SessionObservabilitySnapshot{
			...
			ActiveAgents:    1,
			QueuedTurns:     2,
			Agents: []team.SessionAgentObservability{{
				Slug:             "fe",
				Status:           "active",
				Activity:         "running",
				QueueDepth:       1,
				ActiveTaskID:     "task-77",
				WorkingDirectory: "/tmp/wuphf-task-77",
			}},
		}
	})
	...
	for _, want := range []string{
		...
		"Provider: codex",
		"@fe · status=active · activity=running · queue=1 · task=task-77 · wd=/tmp/wuphf-task-77",
		"Runtime capabilities:",
		"Memory backend [info]: Shared org memory is currently disabled. Durable state lives in local persistence plus configured cloud backup.",
	} {
	...
	if len(snapshot.Tasks) != 1 || !strings.Contains(filepath.ToSlash(snapshot.Tasks[0].WorktreePath), ".wuphf/task-worktrees/") {
	...
	if snapshot.Observability.Provider != "codex" || snapshot.Observability.ActiveAgents != 1 {
		t.Fatalf("unexpected observability snapshot: %+v", snapshot.Observability)
	}
...
func TestHandleTeamRequestDefaultsEpistemicCheckOptions(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("WUPHF_BROKER_STATE_PATH", filepath.Join(t.TempDir(), "broker-state.json"))
	...
	if _, _, err := handleTeamRequest(context.Background(), nil, TeamRequestArgs{
		...
		Channel:  "general",
		Question: "Are the facts verified?",
		MySlug:   "ceo",
	}); err != nil {
		t.Fatalf("handleTeamRequest: %v", err)
	}
	...
	req := result.Requests[0]
	if req.RecommendedID != "facts_verified" {
		t.Fatalf("expected recommended epistemic option, got %q", req.RecommendedID)
	}
	if len(req.Options) != 4 {
		t.Fatalf("expected default epistemic options, got %+v", req.Options)
	}
	found := false
	for _, option := range req.Options {
		if option.ID == "needs_more_evidence" {
			found = option.RequiresText && strings.TrimSpace(option.TextHint) != ""
		}
	}
	if !found {
		t.Fatalf("expected needs_more_evidence option with text guidance, got %+v", req.Options)
	}
}
...
func TestHandleTeamTaskUsesTaskChannelWhenIDGiven(t *testing.T) {
	...
	result, _, err := handleTeamTask(ctx, nil, TeamTaskArgs{
		...
		ID:     created.Task.ID,
		MySlug: "fe",
		Details: teamMCPTestStructuredHandoff(
			"review-ready",
			"Reviewed launch CTA.",
			"Keep follow-up activity in #launch.",
		),
	})
	if err != nil {
		t.Fatalf("handleTeamTask: %v", err)
...
func teamMCPTestStructuredHandoff(status, summary, downstream string) string {
	return strings.Join([]string{
		"## Task Report",
		"Status: " + status,
		"Summary: " + summary,
		"Touched: internal/teammcp/server_test.go",
		"Validation: go test ./internal/teammcp -run TestHandleTeamTask",
		"",
		"## Downstream Context",
		downstream,
	}, "\n")
...
func TestHandleTeamPlanPreservesExternalWorkspacePath(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	...
	workspace := filepath.Join(t.TempDir(), "external-plan-repo")
	...
	var found *brokerTaskSummary
	for i := range result.Tasks {
		if result.Tasks[i].Title == title {
			found = &result.Tasks[i]
			break
		}
	}
	if found == nil {
		t.Fatalf("expected created task in %+v", result.Tasks)
	}
	if found.ExecutionMode != "external_workspace" || found.WorkspacePath != workspace {
		t.Fatalf("expected external workspace metadata, got %+v", *found)
	}
}
