...
func (s *Store) Create(ch Channel) (*Channel, error) {
	...
	return cloneChannelPointer(s.byID[ch.ID]), nil
}
...
func (s *Store) Get(id string) (*Channel, bool) {
	...
	defer s.mu.RUnlock()
	ch, ok := s.byID[id]
	return cloneChannelPointer(ch), ok
}
...
func (s *Store) GetOrCreateDirect(a, b string) (*Channel, error) {
	...
	s.mu.RUnlock()
	if ok {
		return cloneChannelPointer(existing), nil
	}
...
func (s *Store) FindDirectByMembers(a, b string) (*Channel, bool) {
	...
	return cloneChannelPointer(ch), true
}
...
func cloneChannelPointer(ch *Channel) *Channel {
	if ch == nil {
		return nil
	}
	cp := *ch
