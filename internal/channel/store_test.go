...
func TestStoreGetBySlugReturnsCopy(t *testing.T) {
	s := NewStore()
	if _, err := s.Create(Channel{Name: "general", Slug: "general", Type: ChannelTypePublic}); err != nil {
		t.Fatalf("create: %v", err)
	}
	got, ok := s.GetBySlug("general")
	if !ok {
		t.Fatal("GetBySlug failed")
	}
	got.Name = "mutated without store API"

	again, ok := s.GetBySlug("general")
	if !ok {
		t.Fatal("second GetBySlug failed")
	}
	if again.Name == got.Name {
		t.Fatalf("GetBySlug returned mutable internal channel pointer")
	}
}
