...
func TestGitCommandAddsSafeDirectoryForWorktreePaths(t *testing.T) {
	repoDir := t.TempDir()
	worktreeDir := filepath.Join(repoDir, "task-worktree")
	if err := os.MkdirAll(worktreeDir, 0o755); err != nil {
		t.Fatalf("mkdir worktree dir: %v", err)
	}
	if err := os.WriteFile(filepath.Join(repoDir, ".git"), []byte("gitdir: /tmp/main-worktree\n"), 0o644); err != nil {
		t.Fatalf("write worktree git file: %v", err)
	}

	cmd := gitCommand(worktreeDir, "status", "--short")
	wantArgs := []string{"git", "-c", "safe.directory=" + repoDir, "status", "--short"}
	if strings.Join(cmd.Args, "\n") != strings.Join(wantArgs, "\n") {
		t.Fatalf("git args = %q, want %q", cmd.Args, wantArgs)
	}
	if cmd.Dir != worktreeDir {
		t.Fatalf("git dir = %q, want %q", cmd.Dir, worktreeDir)
	}
}
...
func TestCleanupTaskWorktreeAtRepoRootDoesNotDeleteUnmanagedBranch(t *testing.T) {
	repoDir := t.TempDir()
	...
	if err := cleanupTaskWorktreeAtRepoRoot(repoDir, "", "main"); err != nil {
		t.Fatalf("cleanup unmanaged branch: %v", err)
	}
	if !gitRefExists(repoDir, "refs/heads/main") {
		t.Fatalf("cleanup deleted unmanaged branch main")
	}
}
...
func TestDefaultPrepareTaskWorktreeOverlaysDirtyWorkspace(t *testing.T) {
	...
	oldCurrentRepoRoot := currentTaskRepoRoot
	defer func() { taskWorktreeRootDir = oldTaskRoot }()
	defer func() { currentTaskRepoRoot = oldCurrentRepoRoot }()
	...
	untrackedPath := filepath.Join(repoDir, "docs", "launch-ops", "episode-launch-packets", "vid_01-inbox-operator.yaml")
	...
	lease, err := readTaskWorktreeLease(path)
	if err != nil {
		t.Fatalf("read task worktree lease: %v", err)
	}
	if lease.TaskID != "task-overlay" || lease.Branch != branch || lease.WorktreePath != path {
		t.Fatalf("unexpected worktree lease: %+v", lease)
	}
	if expiresAt, err := time.Parse(time.RFC3339, lease.ExpiresAt); err != nil || !expiresAt.After(time.Now().UTC()) {
		t.Fatalf("expected future lease expiry, got %q (err=%v)", lease.ExpiresAt, err)
	}
	...
	untrackedRaw, err := os.ReadFile(filepath.Join(path, "docs", "launch-ops", "episode-launch-packets", "vid_01-inbox-operator.yaml"))
	if err != nil {
...
func TestTaskWorktreeLeaseExpiryFallsBackToDirectoryModTime(t *testing.T) {
	path := filepath.Join(t.TempDir(), "wuphf-task-legacy")
	if err := os.MkdirAll(path, 0o755); err != nil {
		t.Fatalf("mkdir legacy worktree: %v", err)
	}
	now := time.Now().UTC()
	staleAt := now.Add(-2 * taskWorktreeLeaseTTL)
	if err := os.Chtimes(path, staleAt, staleAt); err != nil {
		t.Fatalf("chtimes legacy worktree: %v", err)
	}
	if !taskWorktreeLeaseExpired(path, now) {
		t.Fatalf("expected legacy worktree without lease metadata to expire from directory age")
	}
}

func TestOverlayPersistedTaskWorktreesSkipsDifferentRepository(t *testing.T) {
	root := t.TempDir()
	...
	for _, repoDir := range []string{currentRepo, otherRepo} {
		if err := os.MkdirAll(repoDir, 0o755); err != nil {
			t.Fatalf("mkdir repo %s: %v", repoDir, err)
		}
		run(repoDir, "git", "init", "-b", "main")
		run(repoDir, "git", "config", "user.name", "WUPHF Test")
		run(repoDir, "git", "config", "user.email", "wuphf@example.com")
	...
	if err := os.WriteFile(filepath.Join(otherRepo, "foreign.txt"), []byte("foreign\n"), 0o644); err != nil {
		t.Fatalf("write foreign marker: %v", err)
	}
	run(otherRepo, "git", "add", "foreign.txt")
		...
		Tasks: []teamTask{
			{
				...
				WorktreeBranch: "wuphf-task-other",
			},
		},
	}
	raw, err := json.Marshal(state)
	...
	if err := overlayPersistedTaskWorktrees(currentRepo, "task-current"); err != nil {
		t.Fatalf("overlayPersistedTaskWorktrees: %v", err)
	}
	if _, err := os.Stat(filepath.Join(currentRepo, "foreign.txt")); !os.IsNotExist(err) {
		t.Fatalf("expected foreign repo marker to be skipped, stat err=%v", err)
	}
}
