...
const (
	...
	autoRecoveryBlockCooldownFactor = 2
)
...
type Launcher struct {
	...
	headlessQuickWorkers map[string]bool
	headlessQuickActive  map[string]*headlessCodexActiveTurn
	headlessQuickQueues  map[string][]headlessCodexTurn
	...
	mentionAckMu       sync.Mutex
	pendingMentionAcks map[string]pendingMentionAck
}
...
func NewLauncher(packSlug string) (*Launcher, error) {
	...
	return &Launcher{
		...
		headlessDeferredLead: make(map[string]*headlessCodexTurn),
...
func (l *Launcher) Preflight() error {
	if l.usesCodexRuntime() {
		if _, _, err := provider.ResolveCodexCLIPath(exec.LookPath); err != nil {
	...
	switch strings.TrimSpace(strings.ToLower(l.provider)) {
	case "", "claude", "claude-code":
		if _, err := exec.LookPath("claude"); err != nil {
			return fmt.Errorf("claude not found. Install: npm install -g @anthropic-ai/claude-code")
		}
	case "gemini":
		if strings.TrimSpace(config.ResolveGeminiAPIKey()) == "" {
			return fmt.Errorf("gemini api key not configured. Set GEMINI_API_KEY or WUPHF_GEMINI_API_KEY")
		}
	}
	return nil
}
...
func checkGHCapability() (installed bool, authed bool, note string) {
	...
	if err := cmd.Run(); err != nil {
		if errors.Is(ctx.Err(), context.DeadlineExceeded) {
			return true, false, "gh auth status timed out; GitHub publication checks will keep running in the background."
		}
...
func (l *Launcher) Launch() error {
	...
	l.broker.SetReadinessSnapshot(brokerReadinessSnapshot{
		Live:    true,
		Ready:   false,
		State:   "starting",
		Stage:   "broker_start",
		Summary: "Subindo broker local.",
	})
...
var (
	agentMentionAckTimeout = 2 * time.Minute
	agentMentionAckTimer   = func(d time.Duration, fn func()) { time.AfterFunc(d, fn) }
)

type pendingMentionAck struct {
	Key             string
	Channel         string
...
func (l *Launcher) deliverMessageNotification(msg channelMessage) {
	l.resolveMentionAcknowledgementsFromMessage(msg)
	...
	isHumanMessage := msg.From == "you" || msg.From == "human" || msg.From == "nex"
	isHumanOrCEO := isHumanMessage || msg.From == l.officeLeadSlug()
	...
	for _, t := range immediate {
		laneKey := agentLaneKey(msg.Channel, t.Slug)
		if laneKey == "" {
			continue
		}
		if cooldown > 0 {
			if last, ok := l.notifyLastDelivered[laneKey]; ok && now.Sub(last) < cooldown {
				continue
			}
		}
		l.notifyLastDelivered[laneKey] = now
	...
	l.registerMentionAcknowledgements(msg, immediate)
...
func (l *Launcher) taskNotificationTargets(action officeActionLog, task teamTask) (immediate []notificationTarget, delayed []notificationTarget) {
	...
	addImmediate := func(slug string) {
		...
		if !l.shouldWakeTargetForTask(slug) {
			return
		}
...
func (l *Launcher) messageCanWakeTarget(msg channelMessage, slug string) bool {
	slug = normalizeChannelSlug(slug)
	if !isOwnerOnlyCallableAgent(slug) {
		return true
	}
	if !isOwnerInvocationSender(msg.From) {
		return false
	}
	if isOwnerOnlyCallableDMTarget(msg.Channel, slug, l.isChannelDM) {
		return true
	}
	return containsSlug(msg.Tagged, slug)
}

func (l *Launcher) shouldWakeTargetForTask(slug string) bool {
	return !isOwnerOnlyCallableAgent(slug)
}
...
func (l *Launcher) taskForAction(action officeActionLog) (teamTask, bool) {
	...
	return l.broker.TaskByID(strings.TrimSpace(action.RelatedID))
}
...
func (l *Launcher) taskNotificationContent(action officeActionLog, task teamTask) string {
	...
	worktree := ""
	if strings.TrimSpace(task.WorktreeBranch) != "" || effectiveTeamTaskWorkspacePath(&task) != "" {
...
func (l *Launcher) notificationTargetsForMessage(msg channelMessage) (immediate []notificationTarget, delayed []notificationTarget) {
	...
	ownerOnlyInvocation := containsSlug(msg.Tagged, ownerOnlyCallableAgentSlug) && isOwnerInvocationSender(msg.From)
	...
	if ch := normalizeChannelSlug(msg.Channel); IsDMSlug(ch) {
		...
		if isOwnerOnlyCallableAgent(agentSlug) && !isOwnerInvocationSender(msg.From) {
			return nil, nil
		}
	...
	specialistMessageNeedsLeadAttention := func(msg channelMessage) bool {
		if !messageIsSubstantiveOfficeContent(strings.TrimSpace(msg.Content)) {
			return false
		}
		if l == nil || l.broker == nil {
			return true
		...
		if messageLooksLikeCoordinationGuidance(msg.Content) {
			for _, prior := range l.broker.AllMessages() {
				if prior.ID == msg.ID {
					continue
				}
				if strings.TrimSpace(prior.From) != strings.TrimSpace(msg.From) {
					continue
				}
				if normalizeChannelSlug(prior.Channel) != normalizeChannelSlug(msg.Channel) {
					continue
				}
				if !messageLooksLikeCoordinationGuidance(prior.Content) {
					continue
				}
				if normalizeCoordinationGuidanceSignature(prior.Content) == normalizeCoordinationGuidanceSignature(msg.Content) || coordinationGuidanceEquivalent(prior.Content, msg.Content) {
					return false
				}
			}
		}
		...
		signature := normalizeBlockedNoDeltaSignature(msg.Content)
		if signature == "" {
			return true
		}
		for _, prior := range l.broker.AllMessages() {
			...
			if strings.TrimSpace(prior.ReplyTo) != strings.TrimSpace(msg.ReplyTo) {
				continue
			}
	...
	if l.isFocusModeEnabled() {
		...
		case msg.From == "you" || msg.From == "human" || msg.Kind == "automation" || msg.From == "nex":
			// Human messages always wake the CEO first. Explicit specialist tags still
			// wake those specialists immediately, but they no longer suppress the lead.
			if !ownerOnlyInvocation {
				addImmediate(lead)
			}
		...
		default:
			...
			// Repeated blocked/no-delta updates are also suppressed to avoid CEO ping-pong.
			if specialistMessageNeedsLeadAttention(msg) {
				addImmediate(lead)
			}
		}
		return immediate, delayed
	...
	default:
		...
		// only for substantive/non-repeated updates, plus any tagged agents and the task owner.
		if specialistMessageNeedsLeadAttention(msg) {
			addImmediate(lead)
		}
		if owner != "" && owner != lead && allowTarget(owner) {
...
func mentionAckKey(channel, sourceMessageID, slug string) string {
	return normalizeChannelSlug(channel) + "|" + strings.TrimSpace(sourceMessageID) + "|" + normalizeChannelSlug(slug)
}

func (l *Launcher) registerMentionAcknowledgements(msg channelMessage, targets []notificationTarget) {
	if l == nil || len(targets) == 0 || strings.TrimSpace(msg.ID) == "" || len(msg.Tagged) == 0 {
		return
	}
	if !l.messageCanCreateMentionAcknowledgement(msg) {
		return
	}
	...
	for _, target := range targets {
		if slug := normalizeChannelSlug(target.Slug); slug != "" {
			delivered[slug] = struct{}{}
		}
	}
	threadRoot := strings.TrimSpace(l.ultimateThreadRoot(channel, msg.ReplyTo))
	if threadRoot == "" {
		threadRoot = strings.TrimSpace(firstNonEmpty(msg.ReplyTo, msg.ID))
	}
	now := time.Now().UTC()
	var keys []string

	l.mentionAckMu.Lock()
	if l.pendingMentionAcks == nil {
		l.pendingMentionAcks = make(map[string]pendingMentionAck)
	}
	for _, rawSlug := range msg.Tagged {
		slug := normalizeChannelSlug(rawSlug)
		if slug == "" || slug == "all" || slug == normalizeChannelSlug(msg.From) {
			continue
		}
		if _, ok := delivered[slug]; !ok {
			continue
		}
		key := mentionAckKey(channel, msg.ID, slug)
		if _, exists := l.pendingMentionAcks[key]; exists {
			continue
		}
		l.pendingMentionAcks[key] = pendingMentionAck{
			Key:             key,
			Channel:         channel,
			SourceMessageID: strings.TrimSpace(msg.ID),
			ThreadRootID:    threadRoot,
			MentionedSlug:   slug,
			MentionedBy:     strings.TrimSpace(msg.From),
			SourceContent:   strings.TrimSpace(msg.Content),
			CreatedAt:       now,
			DueAt:           now.Add(agentMentionAckTimeout),
		}
		keys = append(keys, key)
	}
	l.mentionAckMu.Unlock()

	for _, key := range keys {
		key := key
		agentMentionAckTimer(agentMentionAckTimeout, func() {
			l.handleMentionAckTimeout(key)
		})
	}
}

func (l *Launcher) messageCanCreateMentionAcknowledgement(msg channelMessage) bool {
	from := strings.TrimSpace(msg.From)
	return from == "you" || from == "human" || from == "nex" || msg.Kind == "automation" || from == l.officeLeadSlug()
}

func (l *Launcher) resolveMentionAcknowledgementsFromMessage(msg channelMessage) {
	if l == nil || strings.TrimSpace(msg.From) == "" || isHumanLikeActor(msg.From) || isSystemActor(msg.From) {
		return
	}
	channel := normalizeChannelSlug(msg.Channel)
	if channel == "" {
	...
	threadRoot := ""
	if replyTo != "" {
		threadRoot = strings.TrimSpace(l.ultimateThreadRoot(channel, replyTo))
		if threadRoot == "" {
			threadRoot = replyTo
		}
	}
	...
	l.mentionAckMu.Lock()
	defer l.mentionAckMu.Unlock()
	for key, pending := range l.pendingMentionAcks {
		if pending.Channel != channel || pending.MentionedSlug != normalizeChannelSlug(msg.From) {
			continue
		}
		if mentionAckMessageMatchesThread(pending, replyTo, threadRoot) {
			delete(l.pendingMentionAcks, key)
		}
	}
}

func mentionAckMessageMatchesThread(pending pendingMentionAck, replyTo, threadRoot string) bool {
	replyTo = strings.TrimSpace(replyTo)
	threadRoot = strings.TrimSpace(threadRoot)
	if replyTo == "" {
		return false
	}
	return replyTo == pending.SourceMessageID ||
		replyTo == pending.ThreadRootID ||
		(threadRoot != "" && threadRoot == pending.ThreadRootID)
}

func (l *Launcher) handleMentionAckTimeout(key string) {
	if l == nil {
		return
	}
	l.mentionAckMu.Lock()
	pending, ok := l.pendingMentionAcks[key]
	if !ok {
		l.mentionAckMu.Unlock()
		return
	}
	...
	if l.mentionAcknowledged(pending) {
		return
	}
	if l.mentionTargetAppearsActive(pending.MentionedSlug, pending.Channel) {
		l.nudgeMentionTarget(pending)
		return
	}
	l.redirectMentionDemand(pending)
}

func (l *Launcher) mentionAcknowledged(pending pendingMentionAck) bool {
	if l == nil || l.broker == nil {
		return false
	}
	for _, msg := range l.broker.ChannelMessages(pending.Channel) {
		if normalizeChannelSlug(msg.From) != pending.MentionedSlug {
			continue
		}
		replyTo := strings.TrimSpace(msg.ReplyTo)
		threadRoot := ""
		if replyTo != "" {
			threadRoot = strings.TrimSpace(l.ultimateThreadRoot(pending.Channel, replyTo))
			if threadRoot == "" {
				threadRoot = replyTo
			}
		}
...
func (l *Launcher) mentionTargetAppearsActive(slug, channel string) bool {
	if l == nil {
		return false
	}
	laneKey := agentLaneKey(channel, slug)
	l.headlessMu.Lock()
	active := l.headlessActive[laneKey] != nil ||
		l.headlessQuickActive[laneKey] != nil
	l.headlessMu.Unlock()
	if active {
		return true
	}
	if l.broker != nil {
		l.broker.mu.Lock()
		activity := l.broker.activity[laneKey]
		l.broker.mu.Unlock()
		if agentActivityLooksActive(activity) {
			return true
		}
	}
	if !l.usesCodexRuntime() && !l.webMode {
		_, ok := l.agentPaneTargets()[normalizeChannelSlug(slug)]
		return ok
	}
	return false
}

func agentActivityLooksActive(activity agentActivitySnapshot) bool {
	status := strings.ToLower(strings.TrimSpace(activity.Status))
	switch status {
	case "active", "running":
		if last := strings.TrimSpace(activity.LastTime); last != "" {
			if ts, err := time.Parse(time.RFC3339, last); err == nil && time.Since(ts) > agentMentionAckTimeout*2 {
				return false
			}
		}
		return true
...
func (l *Launcher) nudgeMentionTarget(pending pendingMentionAck) {
	target, ok := l.agentPaneTargets()[pending.MentionedSlug]
	if !ok {
		return
	}
	msg := channelMessage{
		ID:      pending.SourceMessageID,
		From:    "watchdog",
		Channel: pending.Channel,
		Kind:    "mention_ack_nudge",
		Content: fmt.Sprintf("@%s has not acknowledged the mention yet but appears active. Acknowledge receipt now, then continue the demand: %s", pending.MentionedSlug, truncate(pending.SourceContent, 700)),
		Tagged:  []string{pending.MentionedSlug},
		ReplyTo: pending.SourceMessageID,
	}
	l.sendChannelUpdate(target, msg)
}

func (l *Launcher) redirectMentionDemand(pending pendingMentionAck) {
	fallback := l.bestMentionFallback(pending)
	if fallback == "" {
		fallback = l.officeLeadSlug()
	}
	if fallback == "" || fallback == pending.MentionedSlug {
		return
	}
	if l.broker != nil {
		l.broker.PostSystemMessage(pending.Channel,
			fmt.Sprintf("@%s did not acknowledge a direct mention in time; redirecting the demand to @%s.", pending.MentionedSlug, fallback),
			"mention_ack_timeout",
		)
	}
	target, ok := l.agentPaneTargets()[fallback]
	if !ok {
		return
	}
	msg := channelMessage{
		...
		From:    "watchdog",
		Channel: pending.Channel,
		Kind:    "mention_redirect",
		Content: fmt.Sprintf("@%s did not acknowledge this demand in time. Redirected from @%s to @%s. Original demand from @%s: %s", pending.MentionedSlug, pending.MentionedSlug, fallback, pending.MentionedBy, truncate(pending.SourceContent, 900)),
		Tagged:  []string{fallback},
		ReplyTo: pending.SourceMessageID,
	}
	l.sendChannelUpdate(target, msg)
}

func (l *Launcher) bestMentionFallback(pending pendingMentionAck) string {
	if l == nil {
		return ""
	}
	enabled := l.enabledMentionFallbackCandidates(pending.Channel)
	if len(enabled) == 0 {
		return ""
	}
	msg := channelMessage{
		ID:      pending.SourceMessageID,
		From:    pending.MentionedBy,
		Channel: pending.Channel,
		Content: pending.SourceContent,
	}
	lead := l.officeLeadSlug()
	bestSlug := ""
	bestScore := 0.0
	for _, slug := range enabled {
		slug = normalizeChannelSlug(slug)
		if slug == "" || slug == pending.MentionedSlug || slug == normalizeChannelSlug(pending.MentionedBy) || isOwnerOnlyCallableAgent(slug) {
			continue
		}
		score := l.scoreMessageForAgent(msg, slug)
		if score > bestScore {
			bestScore = score
			bestSlug = slug
		}
	}
	if bestSlug != "" && bestScore >= officeRoutingMatchThreshold {
		return bestSlug
	}
	for _, slug := range enabled {
		slug = normalizeChannelSlug(slug)
		if slug != "" && slug != pending.MentionedSlug && slug != lead && slug != normalizeChannelSlug(pending.MentionedBy) && !isOwnerOnlyCallableAgent(slug) {
			return slug
		}
	}
	if lead != "" && lead != pending.MentionedSlug {
		return lead
	}
	return bestSlug
}

func (l *Launcher) enabledMentionFallbackCandidates(channel string) []string {
	if l == nil {
		return nil
	}
	if l.broker != nil {
		if members := l.broker.EnabledMembers(channel); len(members) > 0 {
			return members
		}
	}
...
func (l *Launcher) watchdogSchedulerLoop() {
	...
	_ = l.broker.EnsureGitHubPublicationAuditJob()
	_ = l.broker.EnsureTaskWorktreeAuditJob()
	_ = l.broker.EnsureCEOConversationFollowUpAuditJob()
...
func (l *Launcher) processDueSchedulerJobs() {
	...
	for _, job := range jobs {
		...
		case taskGitHubAuditTargetType:
			l.processDueGitHubPublicationJob(job)
		case taskWorktreeAuditTargetType:
			l.processDueTaskWorktreeAuditJob(job)
		case ceoConversationFollowUpJobTargetType:
			l.processDueCEOConversationFollowUpJob(job)
		default:
...
func (l *Launcher) processDueCEOConversationFollowUpJob(job schedulerJob) {
	if l.broker == nil {
		return
	}
	_, _, _ = l.broker.SyncCEOConversationFollowUpTasks(time.Now().UTC())
	l.deliverActiveCEOConversationFollowUpNotifications()
	interval := time.Duration(job.IntervalMinutes) * time.Minute
	if interval <= 0 {
		interval = ceoConversationFollowUpInterval
	}
	nextRun := time.Now().UTC().Add(interval)
	_ = l.broker.UpdateSchedulerJobState(job.Slug, nextRun, "scheduled")
}

func (l *Launcher) processDueGitHubPublicationJob(job schedulerJob) {
	if l == nil || l.broker == nil {
		return
	}
	_, _ = l.broker.ReconcileTaskGitHubPublications(time.Now().UTC())
	interval := time.Duration(job.IntervalMinutes) * time.Minute
	if interval <= 0 {
		interval = taskGitHubAuditInterval
	}
	nextRun := time.Now().UTC().Add(interval)
	_ = l.broker.UpdateSchedulerJobState(job.Slug, nextRun, "scheduled")
}

func (l *Launcher) processDueTaskWorktreeAuditJob(job schedulerJob) {
	if l == nil || l.broker == nil {
		return
	}
	_, _ = l.broker.AuditTaskWorktrees(time.Now().UTC())
	interval := time.Duration(job.IntervalMinutes) * time.Minute
	if interval <= 0 {
		interval = taskWorktreeAuditInterval
	}
	nextRun := time.Now().UTC().Add(interval)
	_ = l.broker.UpdateSchedulerJobState(job.Slug, nextRun, "scheduled")
}

func (l *Launcher) deliverActiveCEOConversationFollowUpNotifications() {
	if l == nil || l.broker == nil {
		return
	}
	delivered := make(map[string]struct{})
	for _, task := range l.broker.TasksByType("follow_up") {
		if taskIsTerminal(&task) {
			continue
		}
		executionKey := normalizeExecutionKey(task.ExecutionKey)
		if strings.TrimSpace(task.TaskType) != "follow_up" || !strings.HasPrefix(executionKey, ceoConversationFollowUpTaskPrefix+"|") {
			continue
		}
		if _, exists := delivered[task.ID]; exists {
			continue
		}
		delivered[task.ID] = struct{}{}
		action := officeActionLog{
			Kind:      "watchdog_alert",
			Source:    "watchdog",
			Channel:   normalizeChannelSlug(task.Channel),
...
func (l *Launcher) processDueTaskJob(job schedulerJob) {
	...
	now := time.Now().UTC()
	// Most blocked tasks are legitimately waiting on dependencies — skip the watchdog
	...
	// task_unblocked mechanism wakes them when deps clear. A narrow exception
	// exists for automatic timeout/error recovery blocks that have cooled down.
	if task.Blocked {
		if l.shouldAutoResumeBlockedTask(task, now) {
			resumeNote := "Automatic recovery cooldown expired; resuming automatically for retry."
...
func (l *Launcher) shouldAutoResumeBlockedTask(task teamTask, now time.Time) bool {
	if !task.Blocked {
		return false
	}
	...
	if !isAutomaticRecoveryBlock(task.Details) {
		return false
	}
	if l.broker != nil {
		l.broker.mu.Lock()
		deferredHasUnresolved := l.broker.hasUnresolvedDepsLocked(&task)
		l.broker.mu.Unlock()
		if deferredHasUnresolved {
			return false
		}
	}
	...
	cooldown := time.Duration(autoRecoveryBlockCooldownFactor*config.ResolveTaskReminderInterval()) * time.Minute
	return now.Sub(updated) >= cooldown
}

func isAutomaticRecoveryBlock(details string) bool {
	detail := strings.ToLower(strings.TrimSpace(details))
	return strings.Contains(detail, "automatic timeout recovery") || strings.Contains(detail, "automatic error recovery")
}
...
func (l *Launcher) processDueRequestJob(job schedulerJob) {
	...
	now := time.Now().UTC()
	if escalatedReq, escalated, err := l.broker.EscalateRequestIfDue(req.ID, now); err == nil {
		req = escalatedReq
		if escalated {
			// Reloaded request carries the durable escalation stage and timestamps.
		}
	}
	alertKind := "request_waiting"
	...
	if req.EscalationCount > 0 {
		alertKind = "request_sla_breach"
		summary = fmt.Sprintf("Request SLA %s for %s in #%s: %s", req.EscalationState, req.TitleOrDefault(), normalizeChannelSlug(req.Channel), truncate(req.Question, 120))
	}
	alert, existing, _ := l.broker.CreateWatchdogAlert(alertKind, req.Channel, "request", req.ID, req.From, summary)
	signalIDs, decisionID := l.recordWatchdogLedger(req.Channel, alertKind, req.ID, req.From, summary, "")
	_ = l.broker.RecordAction("watchdog_alert", "watchdog", req.Channel, "watchdog", truncate(summary, 140), req.ID, signalIDs, decisionID)
	if req.Blocking && (!existing || req.EscalationCount > 0) {
	...
	nextRun := now.Add(time.Duration(config.ResolveTaskReminderInterval()) * time.Minute)
	_ = l.broker.UpdateSchedulerJobState(job.Slug, nextRun, "scheduled")
}
...
func (l *Launcher) buildNotificationContext(channel, triggerMsgID, threadRootID string, limit int) string {
	return l.buildNotificationContextForOwner(channel, triggerMsgID, threadRootID, "", limit)
}

func (l *Launcher) buildNotificationContextForOwner(channel, triggerMsgID, threadRootID, owner string, limit int) string {
	if l.broker == nil {
		return ""
	}
	...
	if strings.TrimSpace(owner) != "" {
		if branchIDs := l.broker.ExecutionBranchMessageIDs(channel, triggerMsgID, threadRootID, owner); len(branchIDs) > 0 {
			var branch []channelMessage
			var rootMsg *channelMessage
			rootID := strings.TrimSpace(threadRootID)
			if rootID == "" {
				rootID = strings.TrimSpace(l.ultimateThreadRoot(channel, triggerMsgID))
			}
			for i := range msgs {
				m := &msgs[i]
				if !baseFilter(*m) {
			...
			if rootMsg != nil || len(branch) > 0 {
				...
				if len(branch) > remaining {
					branch = branch[len(branch)-remaining:]
				}
				var items []channelMessage
				if rootMsg != nil {
					items = append(items, *rootMsg)
				}
...
func (l *Launcher) relevantTaskForTarget(msg channelMessage, slug string) (teamTask, bool) {
	...
	msgWorkspacePath := explicitWorkspacePathForMessage(msg)
	threadRoot := strings.TrimSpace(msg.ID)
	if replyTo := strings.TrimSpace(msg.ReplyTo); replyTo != "" {
		threadRoot = firstNonEmpty(l.ultimateThreadRoot(msg.Channel, replyTo), replyTo)
	}
	// Keep matching channel-scoped. Cross-channel matching leaks active work from
	// one lane into another and makes specialists talk about the wrong task.
	var domainOwned teamTask
	bestOwnedScore := 0.0
	for _, task := range l.broker.ChannelTasks(msg.Channel) {
		...
		taskThreadRoot := firstNonEmpty(l.ultimateThreadRoot(msg.Channel, strings.TrimSpace(task.ThreadID)), strings.TrimSpace(task.ThreadID))
		taskSourceRoot := firstNonEmpty(l.ultimateThreadRoot(msg.Channel, strings.TrimSpace(task.SourceMessageID)), strings.TrimSpace(task.SourceMessageID))
		if taskThreadRoot != "" && (taskThreadRoot == strings.TrimSpace(msg.ID) || taskThreadRoot == threadRoot) {
			return task, true
		}
		if taskSourceRoot != "" && (taskSourceRoot == strings.TrimSpace(msg.ID) || taskSourceRoot == threadRoot) {
			return task, true
		}
	...
	if domainOwned.ID != "" {
		if threadRoot != "" {
			l.broker.linkTaskToSourceMessage(domainOwned.ID, threadRoot)
			if linked, ok := l.broker.TaskByID(domainOwned.ID); ok {
				return linked, true
			}
		}
...
func (l *Launcher) responseInstructionForTarget(msg channelMessage, slug string) string {
	lead := l.officeLeadSlug()
	if isOwnerOnlyCallableAgent(slug) {
		if isOwnerOnlyCallableDMTarget(msg.Channel, slug, l.isChannelDM) && isOwnerInvocationSender(msg.From) {
			return fmt.Sprintf("You are @%s. The office owner is messaging you directly in a DM. You have maximum authority. Act on the request, do not delegate implicitly, do not join swarm coordination, and stop once this invocation is complete.", slug)
		}
		if containsSlug(msg.Tagged, slug) && isOwnerInvocationSender(msg.From) {
			return fmt.Sprintf("You are @%s. The office owner invoked you explicitly in-channel. You have maximum authority. Act only on this request, do not join autonomous routing, and stop once this invocation is complete.", slug)
		}
		return fmt.Sprintf("You are @%s. Stay quiet. Only the office owner may invoke you explicitly in-channel or via DM.", slug)
	}
	if slug == lead {
	...
	if containsSlug(msg.Tagged, slug) {
		return fmt.Sprintf("You are @%s. You were directly tagged. Acknowledge receipt immediately in-thread, then reply only from your domain with concrete progress, a blocker, or a handoff.", slug)
	}
...
func (l *Launcher) buildMessageWorkPacket(msg channelMessage, slug string) string {
	...
	if slug == l.officeLeadSlug() {
		...
		l.headlessMu.Lock()
		for laneKey, queue := range l.headlessQueues {
			workerSlug := agentLaneSlug(laneKey)
			if workerSlug == slug || !agentLaneMatchesChannel(laneKey, channel) {
				continue
			}
			if len(queue) > 0 {
		...
		for laneKey, active := range l.headlessActive {
			workerSlug := agentLaneSlug(laneKey)
...
func (l *Launcher) buildTaskExecutionPacket(slug string, action officeActionLog, task teamTask, content string) string {
	...
	if task.ThreadID != "" && (strings.EqualFold(strings.TrimSpace(task.TaskType), "follow_up") || strings.EqualFold(strings.TrimSpace(task.PipelineID), "follow_up")) {
		lines = append(lines, fmt.Sprintf("Follow-up reply rule: answer in-thread via team_broadcast with my_slug \"%s\", channel \"%s\", reply_to_id \"%s\". Once you have posted the needed answer, STOP and wait for the next pushed notification.\n[WUPHF_REPLY_ROUTE channel=\"%s\" reply_to_id=\"%s\"]", slug, channel, task.ThreadID, channel, task.ThreadID))
	}
	lines = append(lines, fmt.Sprintf("- Mutation channel: use #%s when claiming or completing #%s", channel, task.ID))
	if path := effectiveTeamTaskWorkspacePath(&task); path != "" {
		lines = append(lines, fmt.Sprintf("- Working directory: %q", path))
	}
	if openSpec := detectOpenSpecForTeamTask(&task); openSpec.Available() {
		lines = append(lines, openSpec.TaskPacketLines()...)
	}
	if strings.EqualFold(strings.TrimSpace(task.ExecutionMode), "local_worktree") || strings.EqualFold(strings.TrimSpace(task.ExecutionMode), "external_workspace") {
		lines = append(lines, "Execution rule: this is a code workspace build task. Work inside the assigned working_directory and default to direct implementation. Do not spend this turn on another repo audit, architecture memo, or nested office launch unless the packet explicitly asks for that.")
		...
		lines = append(lines, "Dirty-tree rule: ignore unrelated modified or untracked files already present in the working_directory unless they are directly required for your slice. They may be preexisting repo state, not part of your task.")
		lines = append(lines, "Deliverable rule: a code workspace feature task is not satisfied by another plan, architecture memo, or audit summary unless the packet explicitly says research-only. Land code, scripts, docs for the runnable slice, or a concrete task-state blocker.")
	}
	if taskRequiresRealExternalExecution(&task) {
	...
	if ctx := l.buildNotificationContextForOwner(channel, "", threadRoot, slug, 3); ctx != "" {
		lines = append(lines, ctx)
	}
	if brief := fetchChannelMemoryBrief(channel, strings.TrimSpace(task.Title+" "+task.Details+" "+content), l.broker); brief != "" {
		lines = append(lines, brief)
	}
	lines = append(lines, fmt.Sprintf("If you deliver the substantive result for #%s in this turn, you MUST call team_task complete or review-ready for \"%s\" before any completion post and before you stop. For complete, review, block, or reconcile, the team_task details MUST carry a structured handoff with ## Task Report and ## Downstream Context; use ## Blockers on block and ## Review Findings when reporting review output. When a blocker or finding is a separate follow-up, mark it with New Demand: yes. A channel reply alone does not unblock dependent work, and a completion post without the task mutation is a failure.", task.ID, task.ID))
...
func (l *Launcher) sendChannelUpdate(target notificationTarget, msg channelMessage) {
	...
	if l.isOneOnOne() {
		notification = fmt.Sprintf(
			"[New from @%s]: %s\n%s Reply using team_broadcast with my_slug \"%s\" and channel \"%s\" reply_to_id \"%s\". Once you have posted the needed reply, STOP and wait for the next pushed notification.\n[WUPHF_REPLY_ROUTE channel=\"%s\" reply_to_id=\"%s\"]",
			msg.From, truncate(msg.Content, 1000), l.responseInstructionForTarget(msg, target.Slug), target.Slug, channel, msg.ID, channel, msg.ID,
		)
	} else {
		...
		notification = fmt.Sprintf(
			...
			packet, msg.From, truncate(msg.Content, 1000), l.responseInstructionForTarget(msg, target.Slug), target.Slug, channel, msg.ID, channel, msg.ID,
		)
	}

	if l.usesCodexRuntime() || l.webMode {
		quickReply := l.shouldUseLeadQuickReplyLane(target.Slug, msg)
...
func (l *Launcher) shouldUseLeadQuickReplyLane(targetSlug string, msg channelMessage) bool {
	if l == nil {
		return false
	}
	if strings.TrimSpace(targetSlug) != l.officeLeadSlug() {
		return false
	}
	channel := normalizeChannelSlug(msg.Channel)
	if channel == "" {
	...
	case "you", "human":
		for _, tagged := range msg.Tagged {
			if strings.TrimSpace(tagged) != "" && strings.TrimSpace(tagged) != l.officeLeadSlug() {
				return false
			}
		}
		return true
...
func (l *Launcher) buildPrompt(slug string) string {
	...
	if l.isOneOnOne() {
		...
		sb.WriteString("3. Default to direct, useful conversation with the human. Keep it crisp and professional.\n")
		...
		sb.WriteString("- Be concise, direct, and professional.\n")
		sb.WriteString("- Do not use jokes, banter, sarcasm, or playful language.\n")
		sb.WriteString("- Avoid filler, theatrics, and unnecessary framing.\n")
		...
		sb.WriteString("- Do not go silent and over-research by default. Only inspect files, run tools, or query an external memory/integration backend when the answer genuinely depends on that context.\n")
		sb.WriteString("- If you need a deeper pass, give the human the quick answer first, then continue with the deeper work.\n")
		appendRuntimeFallbackGuidance(&sb)
		return sb.String()
	}
	...
	if slug == lead {
		...
		sb.WriteString("- team_request: Use kind `epistemic_check` when risky work depends on distinguishing observed facts from inference.\n")
		...
		sb.WriteString("9. Once decided, create durable task state first, then broadcast the kickoff and assignments. If you already know multiple owned lanes, prefer one team_plan call over several separate team_task creates. Every created task should set `task_type` and `execution_mode` deliberately instead of relying on inference. When task difficulty or domain calls for a specific runtime, also set `runtime_provider`, `runtime_model`, and for Codex/OpenAI runtimes `reasoning_effort`.\n")
		...
		sb.WriteString("16h. Epistemic rule: if a risky or externally visible step depends on uncertain facts, conflicting evidence, or an unproven diagnosis, raise `team_request` with kind `epistemic_check` before proceeding. Separate observed facts, inferences, missing evidence, and the narrow next safe step.\n")
		...
		sb.WriteString("19. For `team_channel` create/remove calls, set `channel` to the explicit target slug like `launch-ops`; it is not inferred from the current room.\n")
		...
		sb.WriteString("- When you create a new specialist for integration/onboarding work, include the owned integrations directly in that agent's expertise so the roster clearly shows who owns Gmail, Slack, Drive, analytics, or similar lanes\n\n")
		sb.WriteString("STYLE: Be concise, direct, and professional. Use markdown tables/checklists for structured data. Do not use jokes, banter, filler, or theatrical phrasing.\n")
		if noNex {
		...
		sb.WriteString("- team_request: Use kind `epistemic_check` when you need an explicit human call on facts vs inference before a risky next step.\n")
		sb.WriteString("Other tools: team_tasks, team_task_status, team_requests, team_request, team_status, team_members, team_office_members, team_channels, team_channel, team_member, team_channel_member, team_action_guide, team_action_workflow_create, team_action_workflow_schedule, team_action_relays, team_action_relay_event_types, team_action_relay_create, team_action_relay_activate, team_action_relay_events, team_action_relay_event.\n\n")
		if noNex {
		...
		sb.WriteString("11g. Epistemic rule: if your next step is risky and you cannot cleanly separate observed facts from inference, stop and raise `team_request` with kind `epistemic_check` instead of bluffing certainty.\n")
		if codingAgentSlugs[slug] {
			sb.WriteString("11h. When you commit to opening a pull request, actually open it. Run `gh pr create --title \"<short title>\" --body \"<body>\" --head \"<your-branch>\" --base main` via the bash tool. Paste the returned URL into your channel message so the team can click through. Do not claim a PR is open unless the bash output shows a https://github.com/... URL.\n")
		}
		if noNex {
		...
		sb.WriteString("STYLE: Be concise, direct, and professional. Stay in lane. Use markdown tables/checklists for structured data. Do not use jokes, banter, filler, or theatrical phrasing.\n")
		sb.WriteString("Never launch another WUPHF office from inside your turn (`wuphf`, `./wuphf`, `/reset`, or a new browser instance). The office is already running; inspect the current repo and UI instead.\n")
		appendRuntimeFallbackGuidance(&sb)
	}
...
func loadCustomMCPServers(path string) (map[string]any, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("read custom MCP config %q: %w", path, err)
	}
	var cfg struct {
		MCPServers map[string]any `json:"mcpServers"`
	}
	if err := json.Unmarshal(data, &cfg); err != nil {
		return nil, fmt.Errorf("parse custom MCP config %q: %w", path, err)
	}
...
func (l *Launcher) buildMCPServerMap() (map[string]any, error) {
	...
	customServers, err := loadCustomMCPServers(config.ResolveCustomMCPConfigPath())
	if err != nil {
		return nil, err
	}
	...
	if config.ResolveWebSearchProvider() != "brave" {
		delete(servers, "brave-search")
	}
...
func normalizeLinkedRepoRoot(path string) string {
	if normalized, ok := normalizeLinkedRepoPath(path); ok {
		path = normalized
	}
	if root, ok := nearestGitWorkspaceRoot(path); ok {
...
func (l *Launcher) taskExecutionWorkspaceDir(task *teamTask) string {
	if task == nil {
		return ""
	}
	switch strings.ToLower(strings.TrimSpace(task.ExecutionMode)) {
	case "external_workspace":
		if path := normalizeHeadlessWorkspaceDir(task.WorkspacePath); path != "" {
			return path
		}
	case "local_worktree":
		if path := normalizeHeadlessWorkspaceDir(task.WorktreePath); path != "" {
			return path
		}
		if strings.TrimSpace(task.ID) != "" {
			if path, _, err := prepareTaskWorktree(task.ID); err == nil {
				return normalizeHeadlessWorkspaceDir(path)
			}
		}
	default:
		if path := normalizeHeadlessWorkspaceDir(effectiveTeamTaskWorkspacePath(task)); path != "" {
			return path
		}
	}
	if task.RepoContext != nil {
		return normalizeHeadlessWorkspaceDir(task.RepoContext.RootPath)
	}
	return ""
}

func (l *Launcher) taskExecutionRepoRoots(task *teamTask) []string {
	if task == nil {
		return nil
	}
	seen := map[string]struct{}{}
	...
	add := func(path string) {
		root := normalizeLinkedRepoRoot(path)
		if root == "" {
			return
		}
	...
	add(l.taskExecutionWorkspaceDir(task))
	return roots
}

func mcpServerMegamemoryRepoRoot(server any) string {
	cfg, ok := server.(map[string]any)
	if !ok {
		return ""
	}
	...
	dbPath, _ := env["MEGAMEMORY_DB_PATH"].(string)
	dbPath = filepath.Clean(strings.TrimSpace(dbPath))
	if dbPath == "" {
		return ""
	}
	return normalizeLinkedRepoRoot(filepath.Dir(filepath.Dir(dbPath)))
}

func isMegamemoryServer(name string, server any) bool {
	return strings.HasPrefix(strings.TrimSpace(name), "megamemory-") || mcpServerMegamemoryRepoRoot(server) != ""
}

func (l *Launcher) scopedLinkedRepoRoots(slug string, channel string, workspaceDir string) []string {
	seen := map[string]struct{}{}
	...
	if l != nil {
		if task := l.agentActiveTask(slug, channel); task != nil {
			for _, root := range l.taskExecutionRepoRoots(task) {
				add(root)
			}
		}
	}
	add(workspaceDir)
	if l != nil && l.broker != nil {
		for _, path := range l.broker.channelLinkedRepoPaths(channel) {
			add(path)
		}
	}
	return roots
...
func (l *Launcher) buildScopedMCPServerMap(slug string, channel string, workspaceDir string) (map[string]any, error) {
	...
	filtered := make(map[string]any, len(allServers))
	...
	linkedRoots := l.scopedLinkedRepoRoots(slug, channel, workspaceDir)
	linkedSet := make(map[string]struct{}, len(linkedRoots))
	for _, root := range linkedRoots {
		linkedSet[root] = struct{}{}
	}

	for key, srv := range allServers {
		...
		if isMegamemoryServer(key, srv) {
			repoRoot := mcpServerMegamemoryRepoRoot(srv)
			if _, ok := linkedSet[repoRoot]; ok {
				filtered[key] = srv
			}
			continue
		}
...
func (l *Launcher) ensureAgentMCPConfigForContext(slug string, channel string, workspaceDir string) (string, error) {
	filtered, err := l.buildScopedMCPServerMap(slug, channel, workspaceDir)
	if err != nil {
		return "", err
	}
...
func (l *Launcher) ensureAgentMCPConfig(slug string) (string, error) {
	return l.ensureAgentMCPConfigForContext(slug, "", "")
}

// agentActiveTask returns the first in_progress task owned by the given agent slug.
// When channel is provided, the lookup stays inside that lane; otherwise it falls
// back to the first active task across channels.
func (l *Launcher) agentActiveTask(slug string, channel ...string) *teamTask {
	if l.broker == nil {
		return nil
	}
	slug = normalizeAgentLaneSlug(slug)
	if task, ok := l.broker.ActiveTaskByOwner(slug, channel...); ok {
		cp := task
		return &cp
	}
	return nil
...
func teamVoiceForSlug(slug string) string {
	switch slug {
	case "ceo":
		return "Executive and decisive. Prioritizes clearly, delegates precisely, and closes decisions without theatrics."
	case "pm":
		return "Structured and analytical. Clarifies scope, surfaces tradeoffs, and removes ambiguity without unnecessary commentary."
	case "fe":
		return "Focused on interface quality, usability, and implementation detail with disciplined technical communication."
	case "be":
		return "Systems-minded and pragmatic. Protects reliability, simplicity, and maintainability with clear technical judgment."
	case "ai":
		return "Pragmatic about model behavior, evaluation, latency, and production reliability. Avoids speculative claims."
	case "designer":
		return "Focused on clarity, hierarchy, usability, and visual quality. Communicates design concerns in direct operational terms."
	case "cmo":
		return "Market-oriented and disciplined. Translates product work into positioning and narrative without exaggeration."
	case "cro":
		return "Commercial and results-focused. Prefers concrete demand signals, objections, and revenue consequences."
	case "tech-lead":
		return "Measured senior engineering judgment. Evaluates architecture, execution risk, and technical tradeoffs with precision."
	case "qa":
		return "Methodical and risk-aware. Tests assumptions, finds edge cases, and communicates defects without drama."
	case "ae":
		return "Polished and commercially focused. Keeps communication clear, credible, and oriented toward progress."
	case "sdr":
		return "Persistent and structured. Keeps outreach focused, disciplined, and aligned to concrete next steps."
	case "research":
		return "Analytical and evidence-driven. Prefers sourced conclusions and clearly separates fact from inference."
	case "content":
		return "Clear and deliberate writer. Optimizes for precision, readability, and useful communication."
	default:
		return "Professional, direct, and concise. Communicates with clear judgment and no unnecessary theatrics."
	}
}

func appendRuntimeFallbackGuidance(sb *strings.Builder) {
	if sb == nil {
		return
	}
	sb.WriteString("RUNTIME/TOOLING RULE: a missing filesystem MCP server or unavailable filesystem MCP resource method is not proof the repo is inaccessible. If the assigned working_directory exists, continue with local shell/file inspection there and report the MCP failure as a runtime limitation, not an office-wide blocker.\n")
}
...
func (l *Launcher) warmRuntimeReadiness() {
	if l == nil || l.broker == nil {
		return
	}
	l.broker.SetReadinessSnapshot(brokerReadinessSnapshot{
		...
		Summary: "Broker local pronto; verificando runtime e integrações opcionais.",
	})

	providerName := strings.TrimSpace(strings.ToLower(l.provider))
	if providerName == "" {
		providerName = strings.TrimSpace(strings.ToLower(config.ResolveLLMProvider("")))
	}
	...
	case "ollama":
		...
		if err != nil {
			ready = false
			signals = append(signals, "ollama: "+truncate(err.Error(), 180))
		}
	}

	if _, _, note := checkGHCapability(); note != "" {
		signals = append(signals, note)
	}
	...
	if !ready {
		state = "degraded"
		summary = "Broker local pronto, mas o runtime ainda tem dependências pendentes."
	} else if len(signals) > 0 {
		summary = "Broker local pronto; integrações opcionais com pendências."
	}
...
func (l *Launcher) restoreDeferredCloudStateAsync() {
	if l == nil || l.broker == nil || !l.broker.HasDeferredCloudRestore() {
		return
	}
	if restored, err := l.broker.RestoreDeferredCloudState(); err != nil {
		log.Printf("broker: deferred cloud restore failed: %v", err)
	} else if restored {
		log.Printf("broker: deferred cloud restore applied after startup")
	}
}
