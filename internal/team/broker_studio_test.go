...
func fakeOneBinaryPath(dir string) string {
	if runtime.GOOS == "windows" {
		return filepath.Join(dir, "one.cmd")
	}
	return filepath.Join(dir, "one")
}

func writeFakeOperationOne(t *testing.T) string {
	t.Helper()
	dir := t.TempDir()
	path := fakeOneBinaryPath(dir)
	script := `#!/bin/sh
if [ "$1" = "--agent" ]; then
  shift
fi
...
`
	if runtime.GOOS == "windows" {
		script = `@echo off
if "%1"=="--agent" shift
if "%cmd3%"=="flow create sponsor-outreach-dry-run" (
  echo {"created":true,"key":"sponsor-outreach-dry-run","path":"C:\\tmp\\.one\\flows\\sponsor-outreach-dry-run.flow.json"}
  exit /b 0
)
if "%cmd3%"=="flow execute sponsor-outreach-dry-run" (
  echo {"event":"step:start","stepId":"compose-email"}
  echo {"event":"workflow:result","runId":"run-123","logFile":"C:\\tmp\\run.log","status":"success","steps":{"gmail-preview":{"status":"success"}}}
  exit /b 0
)
echo unexpected args: %* 1>&2
exit /b 1
`
		script = strings.ReplaceAll(script, "%cmd3%", "%1 %2 %3")
	}
...
`
	if runtime.GOOS == "windows" {
		script = `@echo off
if "%1"=="--agent" shift
if "%cmd3%"=="flow create consulting-live-gmail-read" (
  echo {"created":true,"key":"consulting-live-gmail-read","path":"C:\\tmp\\.one\\flows\\consulting-live-gmail-read.flow.json"}
  exit /b 0
)
if "%cmd3%"=="flow execute consulting-live-gmail-read" (
  echo {"event":"flow:start","runId":"run-live-1"}
  echo {"event":"step:error","stepId":"fetchEmails","error":"429 User-rate limit exceeded. Retry after 2026-04-14T22:55:02.178Z"} 1>&2
  exit /b 1
)
echo unexpected args: %* 1>&2
exit /b 1
...
func TestHandleMemoryRoundTripScopedStudioRecords(t *testing.T) {
	...
	writeRecord("studio_workspace/workspaces/faceless-foundry", "config", map[string]any{
		"workspace": map[string]any{"name": "Faceless Foundry"},
	})
	writeRecord("studio_workspace/workspaces/faceless-foundry", "runs", []map[string]any{
		{"id": "run-1", "title": "Automate founder ops"},
	})
	writeRecord("studio_workspace/workspaces/faceless-foundry", "offers", []map[string]any{
		{"id": "offer-1", "name": "AI Toolkit"},
	})
	writeRecord("studio_workspace/workspaces/faceless-foundry", "artifacts", map[string]any{
	...
	ns := resp.Memory["studio_workspace/workspaces/faceless-foundry"]
	if ns == nil {
...
func TestBuildOperationBootstrapPackageFromRepoIncludesStarterPlan(t *testing.T) {
	pkg, err := buildOperationBootstrapPackageFromRepo(context.Background(), operationCompanyProfile{
		BlueprintID: "multi-agent-workflow-consulting",
		Name:        "Acme Advisory",
	})
	if err != nil {
		t.Fatalf("buildOperationBootstrapPackageFromRepo: %v", err)
	}
	if pkg.Blueprint.ID != "multi-agent-workflow-consulting" {
	...
	if len(pkg.WorkstreamSeed) == 0 || strings.TrimSpace(pkg.WorkstreamSeed[0].ID) == "" {
	...
	if strings.TrimSpace(pkg.WorkstreamSeed[0].Monetization) == "" {
		t.Fatalf("expected workstream monetization from template, got %+v", pkg.WorkstreamSeed[0])
	}
	if !strings.Contains(filepath.ToSlash(pkg.SourcePath), "templates/operations/multi-agent-workflow-consulting/blueprint.yaml") {
		t.Fatalf("expected template blueprint source path, got %q", pkg.SourcePath)
	}
}

func TestBuildOperationBootstrapPackageFromRepoSynthesizesForUnknownBlueprint(t *testing.T) {
	pkg, err := buildOperationBootstrapPackageFromRepo(context.Background(), operationCompanyProfile{
		BlueprintID: "zzzz-does-not-exist",
		Name:        "Unmapped Operation",
		Description: "No matching operation blueprint should synthesize a generic package.",
	})
	if err != nil {
		t.Fatalf("buildOperationBootstrapPackageFromRepo unknown blueprint: %v", err)
	}
	if pkg.Blueprint.ID == "multi-agent-workflow-consulting" || pkg.SourcePath != "synthesized" {
		t.Fatalf("expected synthesized package for removed blueprint alias, got %+v", pkg)
	}
}
...
func TestBuildOperationBootstrapPackageSynthesizesWhenNoPackSeedExists(t *testing.T) {
	...
	if pkg.Blueprint.Kind == "" || pkg.Blueprint.ID == "multi-agent-workflow-consulting" {
	...
	if len(pkg.Starter.Agents) < 5 || len(pkg.Starter.Channels) != 1 || len(pkg.Starter.Tasks) < 5 {
		t.Fatalf("expected synthesized starter plan, got %+v", pkg.Starter)
	}
	if pkg.Starter.Channels[0].Slug != "general" {
		t.Fatalf("expected synthesized starter plan to keep only general, got %+v", pkg.Starter.Channels)
	}
	for _, task := range pkg.Starter.Tasks {
		if task.Channel != "general" {
			t.Fatalf("expected synthesized starter task in general, got %+v", task)
		}
	}
	if len(pkg.Connections) != len(pkg.Blueprint.Connections) {
