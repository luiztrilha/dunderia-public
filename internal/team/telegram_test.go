...
func TestTelegramInboundSummaryRedactsMessageContent(t *testing.T) {
	msg := &telegramMsg{
		...
		Text: "super secret customer message",
	}

	summary := telegramInboundSummary(msg)
	if strings.Contains(summary, msg.Text) {
		t.Fatalf("expected summary to redact message content, got %q", summary)
	}
	if !strings.Contains(summary, "len=29") {
		t.Fatalf("expected summary to report message length, got %q", summary)
	}
	if !strings.Contains(summary, "@alice_dev") {
		t.Fatalf("expected summary to include sender label, got %q", summary)
	}
}
