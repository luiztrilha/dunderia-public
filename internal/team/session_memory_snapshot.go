...
type SessionMemoryTaskSummary struct {
	...
	RelevantBlockers       []string `json:"relevant_blockers,omitempty"`
...
func BuildSessionMemorySnapshot(sessionMode, directAgent string, tasks []RuntimeTask, requests []RuntimeRequest, recent []RuntimeMessage) SessionMemorySnapshot {
	...
	for _, task := range tasks {
		nextAction := runtimeTaskNextAction(task)
		replyChannel := runtimeTaskReplyChannel(task)
		replyTo := runtimeTaskReplyTo(task)
		relevantBlockers := runtimeTaskResumeBlockers(task)
		snapshot.Tasks = append(snapshot.Tasks, SessionMemoryTaskSummary{
			...
			BlockingReviewFindings: task.BlockingReviewFindings,
			ReconciliationPending:  task.ReconciliationPending,
			ReconciliationReason:   strings.TrimSpace(task.ReconciliationReason),
			ChangedPaths:           append([]string(nil), task.ChangedPaths...),
			UntrackedPaths:         append([]string(nil), task.UntrackedPaths...),
			IssuePublicationStatus: strings.TrimSpace(task.IssuePublicationStatus),
			IssueURL:               strings.TrimSpace(task.IssueURL),
			PRPublicationStatus:    strings.TrimSpace(task.PRPublicationStatus),
			PRURL:                  strings.TrimSpace(task.PRURL),
			NextAction:             nextAction,
			ReplyChannel:           replyChannel,
			ReplyTo:                replyTo,
			RelevantBlockers:       append([]string(nil), relevantBlockers...),
...
func BuildSessionMemorySnapshotFromOfficeState(sessionMode, directAgent string, tasks []teamTask, requests []humanInterview, actions []officeActionLog, messages []channelMessage) SessionMemorySnapshot {
	...
	for _, task := range tasks {
		...
		taskSummaries = append(taskSummaries, SessionMemoryTaskSummary{
			ID:                     strings.TrimSpace(runtimeTask.ID),
			Title:                  strings.TrimSpace(runtimeTask.Title),
			Owner:                  strings.TrimSpace(runtimeTask.Owner),
			Status:                 strings.TrimSpace(runtimeTask.Status),
			PipelineStage:          strings.TrimSpace(runtimeTask.PipelineStage),
			ReviewState:            strings.TrimSpace(runtimeTask.ReviewState),
			ExecutionMode:          strings.TrimSpace(runtimeTask.ExecutionMode),
			WorkspacePath:          strings.TrimSpace(runtimeTask.WorkspacePath),
			WorktreePath:           strings.TrimSpace(runtimeTask.WorktreePath),
			WorktreeBranch:         strings.TrimSpace(runtimeTask.WorktreeBranch),
			ThreadID:               strings.TrimSpace(runtimeTask.ThreadID),
			HandoffStatus:          strings.TrimSpace(runtimeTask.HandoffStatus),
			BlockerRequestIDs:      append([]string(nil), runtimeTask.BlockerRequestIDs...),
			BlockingReviewFindings: runtimeTask.BlockingReviewFindings,
			ReconciliationPending:  runtimeTask.ReconciliationPending,
			ReconciliationReason:   strings.TrimSpace(runtimeTask.ReconciliationReason),
			ChangedPaths:           append([]string(nil), runtimeTask.ChangedPaths...),
			UntrackedPaths:         append([]string(nil), runtimeTask.UntrackedPaths...),
			IssuePublicationStatus: strings.TrimSpace(runtimeTask.IssuePublicationStatus),
			IssueURL:               strings.TrimSpace(runtimeTask.IssueURL),
			PRPublicationStatus:    strings.TrimSpace(runtimeTask.PRPublicationStatus),
			PRURL:                  strings.TrimSpace(runtimeTask.PRURL),
			NextAction:             runtimeTaskNextAction(runtimeTask),
			ReplyChannel:           runtimeTaskReplyChannel(runtimeTask),
			ReplyTo:                runtimeTaskReplyTo(runtimeTask),
			RelevantBlockers:       append([]string(nil), runtimeTaskResumeBlockers(runtimeTask)...),
			Blocked:                runtimeTask.Blocked,
			DependsOn:              append([]string(nil), task.DependsOn...),
			Summary:                summarizeTask(runtimeTask),
		})
	}
...
func publicationStatus(publication *taskGitHubPublication) string {
	if publication == nil {
		return ""
	}
	return strings.TrimSpace(publication.Status)
}

func publicationURL(publication *taskGitHubPublication) string {
	if publication == nil {
		return ""
	}
	return strings.TrimSpace(publication.URL)
}
...
func (s SessionMemorySnapshot) RestorationContext() SessionRestoreContext {
	...
	for _, task := range s.Tasks {
		...
		if path := effectiveSessionMemoryTaskWorkspacePath(task); path != "" {
