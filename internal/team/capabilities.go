...
func probeCodexCapability() CapabilityStatus {
	if _, name, err := provider.ResolveCodexCLIPath(lookPathFn); err != nil {
		...
		if strings.TrimSpace(name) == "codex-lb" {
			detail = "codex-lb is installed."
		}
		return CapabilityStatus{
			Name:   "codex",
