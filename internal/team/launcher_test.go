...
func initScopedTestGitWorktree(t *testing.T, path string) {
	t.Helper()
...
func ensureScopedTestMemberAccess(b *Broker, channel, slug, name string) {
	if b == nil {
		return
	}
...
func TestResetManifestToPackKeepsLocalWriteWhenCloudMirrorFails(t *testing.T) {
	...
	sink := &testBackupSink{failPuts: 1}
	...
	if err := resetManifestToPack(pack); err != nil {
		t.Fatalf("resetManifestToPack should preserve local success when mirror fails: %v", err)
	}
	...
	if manifest.Name != pack.Name {
		t.Fatalf("expected local manifest to persist pack reset, got %+v", manifest)
	}
}
...
func TestEnsureMCPConfigMergesCustomMCPServers(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	customPath := filepath.Join(t.TempDir(), "custom-mcp.json")
	if err := os.WriteFile(customPath, []byte(`{"mcpServers":{"github":{"command":"pwsh","args":["-File","custom-github.ps1"]}}}`), 0o600); err != nil {
	...
	if _, ok := cfg.MCPServers["wuphf-office"]; !ok {
		t.Fatal("expected built-in wuphf-office MCP server entry")
	}
	server, ok := cfg.MCPServers["github"]
	if !ok {
		t.Fatal("expected custom github MCP server entry")
	}
	if server["command"] != "pwsh" {
		t.Fatalf("expected custom github command pwsh, got %#v", server["command"])
	}
}
...
func TestEnsureAgentMCPConfigIncludesCustomMCPServers(t *testing.T) {
	...
	if _, ok := cfg.MCPServers["filesystem"]; !ok {
		t.Fatal("expected custom filesystem MCP server entry for agent config")
	}
}
...
func TestBuildScopedMCPServerMapIncludesOnlyLinkedMegamemoryServers(t *testing.T) {
	...
	customConfig := fmt.Sprintf(`{"mcpServers":{
		...
		filepath.ToSlash(filepath.Join(repoOne, ".megamemory", "knowledge.db")),
		filepath.ToSlash(filepath.Join(repoTwo, ".megamemory", "knowledge.db")),
		filepath.ToSlash(filepath.Join(repoOther, ".megamemory", "knowledge.db")),
	)
	...
	broker := NewBroker()
	ensureTestMemberAccess(broker, "launch-ops", "ceo", "CEO")
	broker.mu.Lock()
	if !broker.upsertLinkedRepoForChannelLocked("launch-ops", repoOne, "human_message", "you") {
		broker.mu.Unlock()
		t.Fatal("expected repo one link")
	}
	if !broker.upsertLinkedRepoForChannelLocked("launch-ops", repoTwo, "task_workspace", "ceo") {
		broker.mu.Unlock()
		t.Fatal("expected repo two link")
	}
	broker.mu.Unlock()

	l := &Launcher{cwd: t.TempDir(), broker: broker}
	servers, err := l.buildScopedMCPServerMap("ceo", "launch-ops", "")
	if err != nil {
		t.Fatalf("buildScopedMCPServerMap: %v", err)
	}
	if _, ok := servers["serena"]; !ok {
		t.Fatal("expected serena server in scoped config")
	}
	if _, ok := servers["filesystem"]; !ok {
		t.Fatal("expected filesystem server in scoped config")
	}
	if _, ok := servers["megamemory-one"]; !ok {
		t.Fatal("expected linked megamemory-one server in scoped config")
	}
	if _, ok := servers["megamemory-two"]; !ok {
		t.Fatal("expected linked megamemory-two server in scoped config")
	}
	if _, ok := servers["megamemory-other"]; ok {
		t.Fatal("did not expect unrelated megamemory server in scoped config")
	}
}

func TestScopedLinkedRepoRootsPreferActiveTaskWorkspaceThenChannelLinks(t *testing.T) {
	isolateBrokerPersistenceEnv(t)

	repoActive := filepath.Join(t.TempDir(), "active-repo")
	repoLinked := filepath.Join(t.TempDir(), "linked-repo")
	initScopedTestGitWorktree(t, repoActive)
	initScopedTestGitWorktree(t, repoLinked)
	...
	broker.channels = []teamChannel{{
		...
		LinkedRepos: []linkedRepoRef{
			{RepoPath: repoLinked, Primary: true},
		},
	}}
	...
	broker.rebuildTaskIndexesLocked()
	broker.mu.Unlock()

	l := &Launcher{cwd: t.TempDir(), broker: broker}
	roots := l.scopedLinkedRepoRoots("ceo", "launch-ops", "")
	if len(roots) < 2 {
		t.Fatalf("expected at least 2 scoped roots, got %+v", roots)
	}
	if !samePath(roots[0], repoActive) {
		t.Fatalf("expected active task repo first, got %+v", roots)
	}
	if !samePath(roots[len(roots)-1], repoLinked) {
		t.Fatalf("expected channel linked repo last, got %+v", roots)
	}
}

func TestBuildScopedMCPServerMapIncludesActiveTaskAndChannelLinkedMegamemoryServers(t *testing.T) {
	isolateBrokerPersistenceEnv(t)
	tmpDir := t.TempDir()
	repoActive := filepath.Join(tmpDir, "active")
	repoLinked := filepath.Join(tmpDir, "linked")
	repoOther := filepath.Join(tmpDir, "other")
	initScopedTestGitWorktree(t, repoActive)
	initScopedTestGitWorktree(t, repoLinked)
	initScopedTestGitWorktree(t, repoOther)

	customPath := filepath.Join(tmpDir, "custom-mcp.json")
	customConfig := fmt.Sprintf(`{"mcpServers":{
		"megamemory-active":{"command":"npx","args":["-y","megamemory"],"env":{"MEGAMEMORY_DB_PATH":%q}},
		"megamemory-linked":{"command":"npx","args":["-y","megamemory"],"env":{"MEGAMEMORY_DB_PATH":%q}},
		"megamemory-other":{"command":"npx","args":["-y","megamemory"],"env":{"MEGAMEMORY_DB_PATH":%q}}
	}}`,
		filepath.ToSlash(filepath.Join(repoActive, ".megamemory", "knowledge.db")),
		filepath.ToSlash(filepath.Join(repoLinked, ".megamemory", "knowledge.db")),
		filepath.ToSlash(filepath.Join(repoOther, ".megamemory", "knowledge.db")),
	)
	if err := os.WriteFile(customPath, []byte(customConfig), 0o600); err != nil {
		t.Fatalf("write custom MCP config: %v", err)
	...
	if _, ok := servers["megamemory-active"]; !ok {
		t.Fatal("expected active task megamemory server in scoped config")
	}
	if _, ok := servers["megamemory-linked"]; !ok {
		t.Fatal("expected linked repo megamemory server in scoped config")
	}
	if _, ok := servers["megamemory-other"]; ok {
		t.Fatal("did not expect unrelated megamemory server in scoped config")
	}
...
func TestDunderIAMCPConfigUsesRepoLocalFilesystemLauncher(t *testing.T) {
	...
	configPath := filepath.Join(repoRoot, "mcp", "dunderia-mcp-settings.json")
	...
	foundScript := false
	for i := 0; i < len(filesystem.Args)-1; i++ {
		if filesystem.Args[i] == "-File" && filepath.ToSlash(filesystem.Args[i+1]) == wantScript {
			foundScript = true
			break
		}
	}
	if !foundScript {
		t.Fatalf("expected filesystem args to invoke %q, got %v", wantScript, filesystem.Args)
	}
	...
	for _, unwanted := range []string{
		...
		for _, arg := range filesystem.Args {
			if arg == unwanted {
				t.Fatalf("expected tracked MCP profile to stay repo-local without hard-coded migration root %q, got %v", unwanted, filesystem.Args)
			}
		}
	}
}

func TestEnsureMCPConfigDropsBraveSearchWhenProviderDisabled(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	customPath := filepath.Join(t.TempDir(), "custom-mcp.json")
	if err := os.WriteFile(customPath, []byte(`{"mcpServers":{"brave-search":{"command":"pwsh","args":["-File","launch_brave_mcp.ps1"]}}}`), 0o600); err != nil {
		t.Fatalf("write custom MCP config: %v", err)
	}
	t.Setenv("WUPHF_CUSTOM_MCP_CONFIG_PATH", customPath)
	t.Setenv("WUPHF_WEB_SEARCH_PROVIDER", "none")
	l := &Launcher{cwd: t.TempDir()}
	...
	if _, ok := cfg.MCPServers["brave-search"]; ok {
		t.Fatal("expected brave-search MCP server to be omitted when provider is disabled")
	}
}
...
func TestChannelPaneLogPaths(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	if got := filepath.ToSlash(channelStderrLogPath()); !strings.Contains(got, ".wuphf/logs/channel-stderr.log") {
		t.Fatalf("unexpected stderr log path: %q", got)
	}
	if got := filepath.ToSlash(channelPaneSnapshotPath()); !strings.Contains(got, ".wuphf/logs/channel-pane.log") {
...
func TestNotificationTargetsForHumanMessageDirectToTaggedSpecialists(t *testing.T) {
	...
	// Human messages have highest response priority: even with explicit specialist
	// tags, the CEO must still wake immediately so the lead can stay in the loop.
	if len(immediate) != 3 {
	...
	for _, want := range []string{"ceo", "fe", "be"} {
		found := false
		for _, s := range slugs {
			if s == want {
				found = true
...
func TestDeliverMessageNotificationBypassesLeadDebounceForRapidHumanMessages(t *testing.T) {
	oldPathFn := brokerStatePath
	tmpDir := t.TempDir()
	brokerStatePath = func() string { return filepath.Join(tmpDir, "broker-state.json") }
	defer func() { brokerStatePath = oldPathFn }()
	...
	if got := len(l.headlessQuickQueues["ceo"]); got != 1 {
		t.Fatalf("expected a single pending quick-reply turn for ceo, got %d", got)
	}
	if got := l.headlessQuickQueues["ceo"][0].Prompt; !strings.Contains(got, "segunda pergunta") {
		t.Fatalf("expected latest human message to replace the pending ceo quick reply, got %q", got)
	}
}
...
func TestNotificationTargetsForGameMasterExplicitOwnerTagIsExclusive(t *testing.T) {
	oldPathFn := brokerStatePath
	tmpDir := t.TempDir()
	brokerStatePath = func() string { return filepath.Join(tmpDir, "broker-state.json") }
	defer func() { brokerStatePath = oldPathFn }()
	...
	immediate, delayed := l.notificationTargetsForMessage(channelMessage{
		From:    "you",
		Channel: "general",
		Content: "@game-master inspect the runtime and fix it directly",
	...
	if len(immediate) != 1 || immediate[0].Slug != "game-master" {
		t.Fatalf("expected only game-master to wake, got %+v", immediate)
	}
}

func TestNotificationTargetsForGameMasterIgnoreNonOwnerWakeAttempts(t *testing.T) {
	oldPathFn := brokerStatePath
	tmpDir := t.TempDir()
	brokerStatePath = func() string { return filepath.Join(tmpDir, "broker-state.json") }
	defer func() { brokerStatePath = oldPathFn }()
	...
	immediate, _ := l.notificationTargetsForMessage(channelMessage{
		From:    "ceo",
		Channel: "general",
		Content: "@game-master please take over",
		Tagged:  []string{"game-master"},
	})
	if len(immediate) != 0 {
		t.Fatalf("expected non-owner channel wake attempt to skip game-master, got %+v", immediate)
	}

	immediateDM, _ := l.notificationTargetsForMessage(channelMessage{
		From:    "human",
		Channel: "dm-game-master",
		Content: "take over",
	})
	if len(immediateDM) != 0 {
		t.Fatalf("expected non-owner DM to game-master to be ignored, got %+v", immediateDM)
	}
}
...
func TestTaskNotificationTargetsSkipGameMaster(t *testing.T) {
	l := &Launcher{
		pack: &agent.PackDefinition{
			LeadSlug: "ceo",
			Agents: []agent.AgentConfig{
		...
		broker: &Broker{
			...
			channels: []teamChannel{{
				Slug:    "general",
				Name:    "general",
				Members: []string{"ceo", "game-master"},
			}},
		},
	}
	...
	if len(immediate) != 0 || len(delayed) != 0 {
		t.Fatalf("expected game-master to be excluded from task wakes, got immediate=%+v delayed=%+v", immediate, delayed)
	}
}

func TestNotificationTargetsForDMChannelFrontendFallbackSlug(t *testing.T) {
	// The React DM router historically synthesized "dm-human-<agent>" when it
	// did not yet know the real channel slug. That malformed legacy slug should
	// still wake the intended agent instead of being routed to "human-<agent>".
	oldPathFn := brokerStatePath
	tmpDir := t.TempDir()
	brokerStatePath = func() string { return filepath.Join(tmpDir, "broker-state.json") }
	defer func() { brokerStatePath = oldPathFn }()
	...
	immediate, delayed := l.notificationTargetsForMessage(channelMessage{
		From:    "you",
		Channel: "dm-human-estagiario",
		Content: "online?",
	})
	if len(immediate) != 1 {
		t.Fatalf("expected 1 immediate target for frontend fallback DM slug, got %d: %+v", len(immediate), immediate)
	}
	if immediate[0].Slug != "estagiario" {
		t.Errorf("expected DM target 'estagiario', got %q", immediate[0].Slug)
	}
	if len(delayed) != 0 {
		t.Errorf("expected 0 delayed targets for frontend fallback DM slug, got %d", len(delayed))
	}
}

func TestResponseInstructionForTargetFrontendFallbackDMChannel(t *testing.T) {
	l := &Launcher{
		pack: &agent.PackDefinition{
			LeadSlug: "ceo",
			Agents: []agent.AgentConfig{
	...
	if strings.Contains(dmInstr, "Stay quiet") {
		t.Errorf("frontend fallback DM instruction should not say stay quiet, got %q", dmInstr)
	}
	if !strings.Contains(dmInstr, "messaging you directly") {
		t.Errorf("frontend fallback DM instruction should indicate direct message, got %q", dmInstr)
	}
}
...
func TestResponseInstructionForTaggedAgentRequiresImmediateAcknowledgement(t *testing.T) {
	l := &Launcher{
		pack: &agent.PackDefinition{
			LeadSlug: "ceo",
			Agents: []agent.AgentConfig{
	...
	got := l.responseInstructionForTarget(channelMessage{
		From:    "you",
		Channel: "general",
		Content: "@fe ajuste o composer",
		Tagged:  []string{"fe"},
	}, "fe")

	if !strings.Contains(strings.ToLower(got), "acknowledge receipt") {
		t.Fatalf("expected tagged-agent instruction to require acknowledgement, got %q", got)
	}
}

func TestMentionAcknowledgementClearsWhenTaggedAgentRepliesInThread(t *testing.T) {
	oldTimer := agentMentionAckTimer
	agentMentionAckTimer = func(time.Duration, func()) {}
	t.Cleanup(func() { agentMentionAckTimer = oldTimer })
	...
	l.registerMentionAcknowledgements(msg, []notificationTarget{{Slug: "fe"}})
	if got := len(l.pendingMentionAcks); got != 1 {
		t.Fatalf("expected one pending mention acknowledgement, got %d", got)
	}

	l.resolveMentionAcknowledgementsFromMessage(channelMessage{
		ID:      "msg-2",
		From:    "fe",
		Channel: "general",
		ReplyTo: "msg-1",
		Content: "Recebido, vou ajustar.",
	})

	if got := len(l.pendingMentionAcks); got != 0 {
		t.Fatalf("expected mention acknowledgement to be cleared, got %d pending", got)
	}
}

func TestMentionAcknowledgementTimeoutRedirectsToQualifiedInactiveFallback(t *testing.T) {
	oldTimer := agentMentionAckTimer
	agentMentionAckTimer = func(time.Duration, func()) {}
	t.Cleanup(func() { agentMentionAckTimer = oldTimer })
	...
	b.mu.Lock()
	b.members = []officeMember{
		{Slug: "ceo", Name: "CEO", Role: "Lead", BuiltIn: true, Expertise: []string{"coordination"}},
		{Slug: "fe", Name: "Frontend Engineer", Role: "Frontend", Expertise: []string{"frontend", "react", "ui"}},
		{Slug: "be", Name: "Backend Engineer", Role: "Backend", Expertise: []string{"backend", "api", "rate limiting"}},
	}
	for i := range b.channels {
		if b.channels[i].Slug == "general" {
			b.channels[i].Members = []string{"ceo", "fe", "be"}
		}
	}
	b.mu.Unlock()
	...
	l.headlessWorkers[agentLaneKey("general", "be")] = true

	msg := channelMessage{
		ID:      "msg-1",
		From:    "you",
		Channel: "general",
		Content: "@fe implemente rate limiting na API do broker",
		Tagged:  []string{"fe"},
	}
	l.registerMentionAcknowledgements(msg, []notificationTarget{{Slug: "fe"}})

	for key := range l.pendingMentionAcks {
		l.handleMentionAckTimeout(key)
	}

	queue := l.headlessQueues[agentLaneKey("general", "be")]
	if len(queue) != 1 {
		t.Fatalf("expected backend fallback to receive redirected demand, got queue %+v", l.headlessQueues)
	}
	if prompt := queue[0].Prompt; !strings.Contains(prompt, "rate limiting") || !strings.Contains(prompt, "did not acknowledge") {
		t.Fatalf("expected redirected prompt to carry original demand and timeout reason, got %q", prompt)
	}
}
...
func TestNotificationTargetsForRepeatedBlockedSpecialistUpdateSkipsLead(t *testing.T) {
	oldPathFn := brokerStatePath
	tmpDir := t.TempDir()
	brokerStatePath = func() string { return filepath.Join(tmpDir, "broker-state.json") }
	defer func() { brokerStatePath = oldPathFn }()
	...
	for i := range b.channels {
		if b.channels[i].Slug == "general" {
			b.channels[i].Members = []string{"ceo", "builder"}
		}
	}
	...
	if containsNotificationTarget(immediate, "ceo") {
		t.Fatalf("expected repeated blocked/no-delta specialist update to skip CEO wake, got %+v", immediate)
	}
}

func TestNotificationTargetsForRepeatedCoordinationGuidanceSkipsLead(t *testing.T) {
	oldPathFn := brokerStatePath
	tmpDir := t.TempDir()
	brokerStatePath = func() string { return filepath.Join(tmpDir, "broker-state.json") }
	defer func() { brokerStatePath = oldPathFn }()
	...
	for i := range b.channels {
		if b.channels[i].Slug == "general" {
			b.channels[i].Members = []string{"ceo", "reviewer"}
		}
	}
	b.messages = append(b.messages, channelMessage{
		ID:        "msg-1",
	...
	if containsNotificationTarget(immediate, "ceo") {
		t.Fatalf("expected repeated coordination guidance to skip CEO wake, got %+v", immediate)
	}
}
...
func TestBuildTaskExecutionPacketLocalWorktreeForbidsNestedOffice(t *testing.T) {
	...
	got := l.buildTaskExecutionPacket("eng", officeActionLog{
		...
		Details:       "Wire `docs/launch-ops/default-channel-pack.yaml` into `cmd/wuphf/main.go` first.",
		Owner:         "eng",
		Status:        "in_progress",
		ExecutionMode: "local_worktree",
	...
	if !strings.Contains(got, "Named file targets: docs/launch-ops/default-channel-pack.yaml, cmd/wuphf/main.go") {
...
func TestBuildNotificationContextForOwnerUsesExecutionBranchInsteadOfParallelSibling(t *testing.T) {
	oldPathFn := brokerStatePath
	tmpDir := t.TempDir()
	brokerStatePath = func() string { return filepath.Join(tmpDir, "broker-state.json") }
	defer func() { brokerStatePath = oldPathFn }()
	...
	ensureTestMemberAccess(b, "general", "researcher", "Researcher")
	ensureTestMemberAccess(b, "general", "reviewer", "Reviewer")
	ensureTestMemberAccess(b, "general", "marketing", "Marketing")
	b.mu.Unlock()
	if err := b.StartOnPort(0); err != nil {
		t.Fatalf("start broker: %v", err)
	}
	...
	root, err := b.PostMessage("you", "general", "Pesquise os concorrentes e escreva o email comercial.", nil, "")
	if err != nil {
		t.Fatalf("post root: %v", err)
	}
	researchKickoff, err := b.PostMessage("ceo", "general", "@researcher pesquise os concorrentes.", []string{"researcher"}, root.ID)
	if err != nil {
		t.Fatalf("post research kickoff: %v", err)
	}
	reviewerKickoff, err := b.PostMessage("ceo", "general", "@reviewer levante os riscos contratuais.", []string{"reviewer"}, root.ID)
	if err != nil {
		t.Fatalf("post reviewer kickoff: %v", err)
	}
	researchReply, err := b.PostMessage("researcher", "general", "Pesquisa: concorrente A promete migracao em 3 dias e B cobra onboarding separado.", nil, researchKickoff.ID)
	if err != nil {
		t.Fatalf("post research reply: %v", err)
	}
	if _, err := b.PostMessage("reviewer", "general", "Risco juridico: revisar a clausula de cancelamento antes do envio.", nil, reviewerKickoff.ID); err != nil {
		t.Fatalf("post reviewer reply: %v", err)
	}
	marketingKickoff, err := b.PostMessage("ceo", "general", "@marketing escreva o email com base na pesquisa.", []string{"marketing"}, researchReply.ID)
	if err != nil {
		t.Fatalf("post marketing kickoff: %v", err)
	}

	l := &Launcher{broker: b}
	ctx := l.buildNotificationContextForOwner("general", marketingKickoff.ID, root.ID, "marketing", 6)

	if !strings.Contains(ctx, "[Recent branch]") {
		t.Fatalf("expected branch-scoped context, got %q", ctx)
	}
	if !strings.Contains(ctx, "Pesquise os concorrentes e escreva o email") {
		t.Fatalf("expected root ask in branch context, got %q", ctx)
	}
	if !strings.Contains(ctx, "Pesquisa: concorrente A promete migracao") {
		t.Fatalf("expected research findings in branch context, got %q", ctx)
	}
	if strings.Contains(ctx, "Risco juridico") {
		t.Fatalf("parallel reviewer branch leaked into marketing context: %q", ctx)
	}
	if strings.Contains(ctx, "escreva o email com base na pesquisa") {
		t.Fatalf("trigger message should be excluded from branch context: %q", ctx)
	}
}

func TestBuildMessageWorkPacketCEOScopesTasksToCurrentChannel(t *testing.T) {
	// Channel isolation: when the CEO is woken in one channel, the work packet must
	// not leak active tasks from a different channel into that conversation.
	oldPathFn := brokerStatePath
	tmpDir := t.TempDir()
	brokerStatePath = func() string { return filepath.Join(tmpDir, "broker-state.json") }
	defer func() { brokerStatePath = oldPathFn }()
	...
	// Wake the CEO from a different channel.
	b.mu.Lock()
	b.channels = append(b.channels, teamChannel{
		Slug:    "engineering",
	...
	b.mu.Unlock()
	msg, err := b.PostMessage("human", "engineering", "Need help in engineering", []string{"ceo"}, "")
	if err != nil {
		t.Fatalf("post engineering message: %v", err)
	}

	packet := l.buildMessageWorkPacket(msg, "ceo")
	if strings.Contains(packet, "general-task") {
		t.Fatalf("CEO packet leaked task from another channel: %q", packet)
	}
}
...
func TestBuildMessageWorkPacketIncludesChannelMemoryBrief(t *testing.T) {
	...
	b.sharedMemory[channelMemoryNamespace("general")] = map[string]string{
		"note-1": encodePrivateMemoryNote(privateMemoryNote{
			Key:       "note-1",
			Title:     "Blocked dependency",
			Content:   "API auth is still blocking the login slice for this channel.",
			Author:    "ceo",
	...
	packet := l.buildMessageWorkPacket(msg, "ceo")
	if !strings.Contains(packet, "== CHANNEL MEMORY ==") {
		t.Fatalf("expected channel memory block in message packet: %q", packet)
	}
	if !strings.Contains(packet, "API auth is still blocking the login slice") {
		t.Fatalf("expected channel memory content in message packet: %q", packet)
	}
}

func TestBuildTaskExecutionPacketIncludesChannelMemoryBrief(t *testing.T) {
	oldPathFn := brokerStatePath
	tmpDir := t.TempDir()
	brokerStatePath = func() string { return filepath.Join(tmpDir, "broker-state.json") }
	defer func() { brokerStatePath = oldPathFn }()
	...
	b.sharedMemory[channelMemoryNamespace("general")] = map[string]string{
		"note-1": encodePrivateMemoryNote(privateMemoryNote{
			...
			Content:   "Keep the login rollout behind API auth until the blocker is cleared.",
			Author:    "ceo",
			CreatedAt: "2026-04-19T12:00:00Z",
			UpdatedAt: "2026-04-19T12:00:00Z",
		}),
	...
	packet := l.buildTaskExecutionPacket("eng", officeActionLog{
		...
		Owner:   "eng",
		Status:  "in_progress",
	}, "Implement the backend slice now.")
	if !strings.Contains(packet, "== CHANNEL MEMORY ==") {
		t.Fatalf("expected channel memory block in task packet: %q", packet)
	}
	if !strings.Contains(packet, "Keep the login rollout behind API auth") {
		t.Fatalf("expected channel memory content in task packet: %q", packet)
	}
}

func TestBuildTaskExecutionPacketPrioritizesLatestHumanAndCEOChannelMemory(t *testing.T) {
	oldPathFn := brokerStatePath
	tmpDir := t.TempDir()
	brokerStatePath = func() string { return filepath.Join(tmpDir, "broker-state.json") }
	defer func() { brokerStatePath = oldPathFn }()
	...
	b.sharedMemory[channelMemoryNamespace("general")] = map[string]string{
		"note-stale-1": encodePrivateMemoryNote(privateMemoryNote{
			Key:       "note-stale-1",
			Title:     "Agent handoff",
			Content:   "RetornaCidadesUF continua sendo o slice principal com build verde no Externo.",
			Author:    "builder",
			CreatedAt: "2026-04-19T23:18:00Z",
			UpdatedAt: "2026-04-19T23:18:00Z",
		}),
		"note-stale-2": encodePrivateMemoryNote(privateMemoryNote{
			Key:       "note-stale-2",
			Title:     "Agent handoff",
			Content:   "Task-1219 segue no eixo RetornaCidadesUF e payload legado.",
			Author:    "builder",
			CreatedAt: "2026-04-19T23:18:10Z",
			UpdatedAt: "2026-04-19T23:18:10Z",
		}),
		"note-stale-3": encodePrivateMemoryNote(privateMemoryNote{
			Key:       "note-stale-3",
			Title:     "Agent handoff",
			Content:   "Externo e CidadeBrasil mantem prioridade operacional neste lane.",
			Author:    "builder",
			CreatedAt: "2026-04-19T23:18:20Z",
			UpdatedAt: "2026-04-19T23:18:20Z",
		}),
		"note-human": encodePrivateMemoryNote(privateMemoryNote{
			Key:       "note-human",
			Title:     "Human request",
			Content:   "A tela TcvRecursoHumanoCadastro.aspx.cs nao existe; parem de usar esse nome.",
			Author:    "you",
			CreatedAt: "2026-04-19T23:19:25Z",
			UpdatedAt: "2026-04-19T23:19:25Z",
		}),
		"note-ceo": encodePrivateMemoryNote(privateMemoryNote{
			Key:       "note-ceo",
			Title:     "CEO handoff",
			Content:   "Enterrem esse arquivo fantasma e foquem no eixo cvPrestacaoContas -> cvRecursoHumano -> cvEncargoRH.",
			Author:    "ceo",
			CreatedAt: "2026-04-19T23:19:39Z",
			UpdatedAt: "2026-04-19T23:19:39Z",
		}),
	}
	b.mu.Unlock()

	l := &Launcher{broker: b}
	packet := l.buildTaskExecutionPacket("builder", officeActionLog{
		Kind:  "task_updated",
		Actor: "ceo",
	}, teamTask{
		ID:      "task-1219",
		Channel: "general",
		Title:   "Validar RetornaCidadesUF",
		Owner:   "builder",
		Status:  "in_progress",
	}, "Continue o slice RetornaCidadesUF no Externo.")
	if !strings.Contains(packet, "A tela TcvRecursoHumanoCadastro.aspx.cs nao existe") {
		t.Fatalf("expected latest human correction in packet: %q", packet)
	}
	if !strings.Contains(packet, "foquem no eixo cvPrestacaoContas -> cvRecursoHumano -> cvEncargoRH") {
		t.Fatalf("expected latest CEO handoff in packet: %q", packet)
	}
}

func TestRelevantTaskForTargetDoesNotCrossChannels(t *testing.T) {
	// Channel isolation: a specialist must not get an "Active task" from another
	// channel just because the title/thread is similar.
	oldPathFn := brokerStatePath
	tmpDir := t.TempDir()
	brokerStatePath = func() string { return filepath.Join(tmpDir, "broker-state.json") }
	defer func() { brokerStatePath = oldPathFn }()
	...
	// CEO delegates in "general", but the task lives in "engineering".
	...
	if found, ok := l.relevantTaskForTarget(ceoMsg, "engineering"); ok {
		t.Fatalf("expected no cross-channel task match, got %+v", found)
	}

	// The task remains present in engineering; the isolation rule is about packet
	// scope, not task deletion.
	...
	// Explicit tag still routes the message to the specialist, but without leaking
	// the task context from another channel.
...
func TestTaskOwnerForMessageDoesNotCrossChannels(t *testing.T) {
	oldPathFn := brokerStatePath
	tmpDir := t.TempDir()
	brokerStatePath = func() string { return filepath.Join(tmpDir, "broker-state.json") }
	defer func() { brokerStatePath = oldPathFn }()
	...
	if _, err := b.PostMessage("you", "general", "Need help improving onboarding copy", nil, ""); err != nil {
		t.Fatalf("post general message: %v", err)
	}
	if _, _, err := b.EnsureTask("engineering", "Improve onboarding copy", "Tune the onboarding wording", "engineering", "ceo", ""); err != nil {
		t.Fatalf("ensure engineering task: %v", err)
	}
	...
	msg := channelMessage{
		...
		From:    "human",
		Channel: "general",
		Content: "Need help improving onboarding copy",
	}

	if owner := l.taskOwnerForMessage(msg); owner != "" {
		t.Fatalf("expected no owner match from another channel, got %q", owner)
	}
}
...
func TestRelevantTaskForTargetIgnoresCanceledTask(t *testing.T) {
	oldPathFn := brokerStatePath
	tmpDir := t.TempDir()
	brokerStatePath = func() string { return filepath.Join(tmpDir, "broker-state.json") }
	defer func() { brokerStatePath = oldPathFn }()
	...
	msg := channelMessage{
		...
		From:    "ceo",
		Channel: "general",
		Content: "@reviewer Inicie imediatamente a revisão abrangente da base de código.",
		Tagged:  []string{"reviewer"},
	}

	if task, ok := l.relevantTaskForTarget(msg, "reviewer"); ok {
		t.Fatalf("expected canceled task to be ignored, got %+v", task)
	}
	packet := l.buildMessageWorkPacket(msg, "reviewer")
	if strings.Contains(packet, "- Active task: #task-") {
		t.Fatalf("expected packet to omit canceled active task, got %q", packet)
	}
	if got := headlessCodexTaskID(packet); got != "" {
		t.Fatalf("expected no task id in packet for canceled task, got %q", got)
	}
}

func TestRelevantTaskForTargetPersistsSourceMessageOnSemanticMatch(t *testing.T) {
	oldPathFn := brokerStatePath
	tmpDir := t.TempDir()
	brokerStatePath = func() string { return filepath.Join(tmpDir, "broker-state.json") }
	defer func() { brokerStatePath = oldPathFn }()
	...
	b := NewBroker()
	task, reused, err := b.EnsureTask("general", "Revisao tecnica abrangente da base", "Produzir relatorio .md priorizado.", "reviewer", "ceo", "")
	if err != nil || reused {
	...
	msg := channelMessage{
		...
		From:    "ceo",
		Channel: "general",
		Content: "@reviewer Realize uma revisão abrangente da base. Produza um relatório .md priorizado.",
		Tagged:  []string{"reviewer"},
	}

	found, ok := l.relevantTaskForTarget(msg, "reviewer")
	if !ok {
		t.Fatal("expected semantic task match for reviewer")
	}
	if found.ID != task.ID {
		t.Fatalf("expected task %s, got %+v", task.ID, found)
	}

	persisted, ok := b.TaskByID(task.ID)
	if !ok {
		t.Fatalf("expected persisted task %s", task.ID)
	}
	if persisted.SourceMessageID != "msg-1" {
		t.Fatalf("expected source_message_id to persist msg-1, got %+v", persisted)
	}
}

func TestRelevantTaskForTargetPrefersExplicitWorkspaceMatch(t *testing.T) {
	oldPathFn := brokerStatePath
	tmpDir := t.TempDir()
	brokerStatePath = func() string { return filepath.Join(tmpDir, "broker-state.json") }
	defer func() { brokerStatePath = oldPathFn }()
	...
	if err != nil || reused {
		t.Fatalf("ensure right task: %v reused=%v", err, reused)
	}
	...
	found, ok := l.relevantTaskForTarget(msg, "reviewer")
	if !ok {
		t.Fatal("expected explicit workspace task match for reviewer")
	}
	if found.ID != right.ID {
		t.Fatalf("expected task %s, got %+v", right.ID, found)
	}

	persistedRight, ok := b.TaskByID(right.ID)
	if !ok || persistedRight.SourceMessageID != "msg-1" {
		t.Fatalf("expected source_message_id on explicit workspace task, got %+v", persistedRight)
	}
	persistedWrong, ok := b.TaskByID(wrong.ID)
	if !ok {
		t.Fatalf("expected wrong task %s to remain persisted", wrong.ID)
	}
	if persistedWrong.SourceMessageID != "" {
		t.Fatalf("expected mismatched workspace task to stay unlinked, got %+v", persistedWrong)
	}
}
...
func TestOfficeChangeTaskNotificationsBackfillGeneratedMemberTask(t *testing.T) {
	...
	b.channels = []teamChannel{
		...
		{Slug: "launch-ops", Name: "Launch Ops", Members: []string{"ceo", "eng", "gtm", "ops"}},
	}
	b.tasks = []teamTask{
		{
			ID:      "task-4",
...
func TestOfficeChangeTaskNotificationsBackfillChannelMembershipTask(t *testing.T) {
	...
	b.channels = []teamChannel{
		...
		{Slug: "launch-ops", Name: "Launch Ops", Members: []string{"ceo", "ops"}},
	}
	b.tasks = []teamTask{
		{
			ID:      "task-4",
	...
	notifications := l.officeChangeTaskNotifications(officeChangeEvent{Kind: "channel_updated", Slug: "launch-ops"})
	if len(notifications) != 1 {
