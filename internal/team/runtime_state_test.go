...
func TestDetectRuntimeCapabilitiesTreatsCodexAsReady(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	...
	got := DetectRuntimeCapabilities()
	codex, ok := got.Registry.Entry(CapabilityKeyCodex)
	if !ok {
		t.Fatal("expected codex capability entry")
	}
	if codex.Level != CapabilityReady || codex.Detail != "codex is installed." {
		t.Fatalf("expected codex capability ready via codex executable, got %+v", codex)
	}
	if office, ok := got.Registry.Entry(CapabilityKeyOfficeRuntime); !ok || office.Level != CapabilityReady {
		t.Fatalf("expected office runtime ready via codex executable, got %+v", office)
	}
}
