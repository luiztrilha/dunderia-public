...
func TestSynthesizeBlueprintDerivesGenericPlanFromDirectiveProfileAndCapabilities(t *testing.T) {
	...
	if blueprint.Starter.LeadSlug != "ceo" {
	...
	if len(blueprint.Starter.Channels) != 1 || blueprint.Starter.Channels[0].Slug != "general" {
		t.Fatalf("expected a single general starter channel, got %+v", blueprint.Starter.Channels)
	}
	if len(blueprint.Starter.Tasks) < 5 {
...
func TestSynthesizeBlueprintFallsBackToBlankOperationsShape(t *testing.T) {
	...
	if len(blueprint.Starter.Channels) != 1 || blueprint.Starter.Channels[0].Slug != "general" {
		t.Fatalf("expected only the general channel in fallback blueprint, got %+v", blueprint.Starter.Channels)
	}
	if len(blueprint.Stages) != 5 {
