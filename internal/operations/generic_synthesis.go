...
func genericInferOperationKind(directive string, profile CompanyProfile, name, description, goals, priority string) string {
	...
	case containsAny(text, "lead gen", "lead-generation", "pipeline", "prospect", "outreach", "gtm", "sales", "demand gen"):
		return "gtm"
	case containsAny(text, "content", "creator", "audience", "newsletter", "media", "podcast", "video"):
		return "content"
...
func genericStarterPlan(kind, name, objective string, input SynthesisInput, integrations []RuntimeIntegration, capabilities []RuntimeCapability) StarterPlan {
	...
	agents := []StarterAgent{
		{Slug: leadSlug, Name: "CEO", Role: "Owns priorities, approvals, and escalation decisions", PermissionMode: "plan", Checked: true, Type: "human", BuiltIn: true, Expertise: []string{"prioritization", "approvals", "escalation"}},
		{Slug: "research-lead", Name: "Research Lead", Role: "Frames the problem, gathers proof, and shapes the first backlog", PermissionMode: "plan", Checked: true, Type: "assistant", BuiltIn: true, Expertise: []string{"discovery", "problem framing", "backlog design"}},
		{Slug: "writer", Name: "Writer", Role: "Turns discoveries into briefs, plans, and durable working drafts", PermissionMode: "auto", Checked: true, Type: "assistant", BuiltIn: true, Expertise: []string{"briefs", "plans", "drafting"}},
		{Slug: "operator", Name: "Operator", Role: "Owns delivery coordination, runbooks, and execution hygiene", PermissionMode: "plan", Checked: true, Type: "assistant", BuiltIn: true, Expertise: []string{"runbooks", "delivery coordination", "execution hygiene"}},
		{Slug: "growth-ops", Name: "Growth Ops", Role: "Owns review loops, adoption signals, and operating cadence", PermissionMode: "plan", Checked: true, Type: "assistant", BuiltIn: true, Expertise: []string{"review cadence", "feedback loops", "operating metrics"}},
		{Slug: "monetization", Name: "Monetization", Role: "Defines offers, value capture, and commercial next steps", PermissionMode: "auto", Checked: true, Type: "assistant", BuiltIn: true, Expertise: []string{"offers", "value capture", "commercial design"}},
	}
	return StarterPlan{
		LeadSlug:                  leadSlug,
...
func genericDefaultChannels(integrations []RuntimeIntegration) []StarterChannel {
	...
	return []StarterChannel{{
		Slug:        "general",
		Name:        "general",
		Description: "Primary coordination channel for the office.",
		Members:     []string{"ceo", "research-lead", "writer", "operator", "growth-ops", "monetization"},
	}}
}

func genericDefaultTasks(objective string, integrations []RuntimeIntegration) []StarterTask {
	tasks := []StarterTask{
		{Channel: "general", Owner: "research-lead", Title: "Build the first operating backlog", Details: genericTruncateText(objective, 160)},
		{Channel: "general", Owner: "writer", Title: "Draft the first operating briefs", Details: "Turn the objective into clear briefs, priorities, and next-step drafts."},
		{Channel: "general", Owner: "operator", Title: "Standardize the execution workflow", Details: "Define the runbook, handoff points, and delivery hygiene for the first loop."},
		{Channel: "general", Owner: "growth-ops", Title: "Set the review cadence", Details: "Define the review loop, success signals, and how the backlog should be updated."},
		{Channel: "general", Owner: "monetization", Title: "Define the first value capture paths", Details: "Map the first useful outcomes to offers, pilots, or other commercial next steps."},
	}
	for _, integration := range integrations {
		if integration.Connected {
			continue
