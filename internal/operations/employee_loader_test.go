...
func TestLoadEmployeeBlueprintReadsReviewerTemplate(t *testing.T) {
	blueprint, err := LoadEmployeeBlueprint(testRepoRoot(t), "reviewer")
	if err != nil {
		t.Fatalf("load failed: %v", err)
	}
	if blueprint.ID != "reviewer" {
		t.Fatalf("unexpected id: %+v", blueprint.ID)
	}
	if blueprint.Role == "" || len(blueprint.Skills) == 0 || len(blueprint.Tools) == 0 {
		t.Fatalf("expected populated reviewer blueprint shape: %+v", blueprint)
	}
}

func TestLoadEmployeeBlueprintReadsBugInvestigatorTemplate(t *testing.T) {
	blueprint, err := LoadEmployeeBlueprint(testRepoRoot(t), "bug-investigator")
	if err != nil {
		t.Fatalf("load failed: %v", err)
	}
	if blueprint.ID != "bug-investigator" {
		t.Fatalf("unexpected id: %+v", blueprint.ID)
	}
	if blueprint.Role == "" || len(blueprint.Responsibilities) == 0 || len(blueprint.ExpectedResults) == 0 {
		t.Fatalf("expected populated bug-investigator blueprint shape: %+v", blueprint)
	}
}
...
func TestListEmployeeBlueprintsReturnsTrackedTemplates(t *testing.T) {
	...
	if got, want := len(blueprints), 5; got != want {
