...
func TestKindSpecificAgentNames(t *testing.T) {
	kinds := []struct {
		...
		{kind: "content", directive: "Build a content engine for creator businesses."},
...
func TestSynthesisEndToEnd(t *testing.T) {
	kindInputs := []struct {
	...
	}{
		{
			kind: "content",
			input: SynthesisInput{
				Directive: "Build a video content factory.",
	...
	for _, tt := range kindInputs {
		t.Run(tt.kind, func(t *testing.T) {
			...
			if len(bp.Starter.Channels) != 1 || bp.Starter.Channels[0].Slug != "general" {
				t.Fatalf("expected only the general starter channel, got %+v", bp.Starter.Channels)
			}
			if len(bp.Starter.Tasks) < 5 {
				t.Fatalf("expected at least 5 tasks, got %d", len(bp.Starter.Tasks))
			}
			if len(bp.Stages) < 5 {
...
func TestSynthesizeAllKinds(t *testing.T) {
	...
	directives := map[string]string{
		"content":    "Build an audience with video content.",
