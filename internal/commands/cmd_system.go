...
func cmdHelp(ctx *SlashContext, args string) error {
	help := "Commands:\n\n" +
		...
		"  /init                  Run setup and restore lightweight local state\n" +
...
func cmdInit(ctx *SlashContext, args string) error {
	if config.ResolveNoNex() {
		ctx.AddMessage("system", "Nex integration is disabled for this session (--no-nex). Restart without --no-nex to run setup.")
		return nil
	}
	...
	syncReport, err := lightstate.SyncDefaultState(config.ResolveCloudBackupSettings())
	if err != nil {
		return err
	}
	...
	if len(syncReport.Restored) > 0 {
		ctx.AddMessage("system", "Restored lightweight local state from cloud backup: "+strings.Join(syncReport.Restored, ", "))
	}
	if len(syncReport.Mirrored) > 0 {
		ctx.AddMessage("system", "Mirrored lightweight local state to cloud backup: "+strings.Join(syncReport.Mirrored, ", "))
	}
	...
	if cfg.APIKey == "" {
		ctx.AddMessage("system", "No workspace API key is configured yet. Run interactive /init to add one.")
	}
