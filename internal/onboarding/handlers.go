...
//
// wrap applies auth to state-changing handlers. Pass nil to leave them open,
// which is useful for direct handler tests.
func RegisterRoutes(
	mux *http.ServeMux,
	completeFn func(task string, skipTask bool) error,
	packSlug string,
	...
	mux.HandleFunc("/onboarding/progress", protected(HandleProgress))
	mux.HandleFunc("/onboarding/complete", protected(makeHandleComplete(completeFn)))
	mux.HandleFunc("/onboarding/validate-key", HandleValidateKey)
	mux.HandleFunc("/onboarding/checklist/dismiss", protected(HandleChecklistDismiss))
	...
	mux.HandleFunc("/onboarding/checklist/", protected(HandleChecklistDone))
}
