...
var (
	atomicReplaceStateFile = atomicfile.Replace
	syncStateTempFile      = func(file *os.File) error { return file.Sync() }
	syncStateDir           = atomicfile.SyncDir
)
...
// and a default checklist — no error is returned in that case. Cloud-backed
// onboarding state is only restored through explicit restore flows; plain
// reads must not skip the wizard on a clean install.
...
func Save(s *State) error {
	...
	tmp, err := os.CreateTemp(dir, ".onboarded-*.json")
	if err != nil {
	...
	if err := syncStateTempFile(tmp); err != nil {
		_ = tmp.Close()
		return fmt.Errorf("onboarding: sync temp: %w", err)
	}
	if err := tmp.Close(); err != nil {
		return fmt.Errorf("onboarding: close temp: %w", err)
	}
	if err := atomicReplaceStateFile(tmpPath, path); err != nil {
		return fmt.Errorf("onboarding: rename temp: %w", err)
	}
	if err := syncStateDir(dir); err != nil {
		return fmt.Errorf("onboarding: sync dir: %w", err)
	}
	// Cloud mirror is best-effort; local onboarding state must remain authoritative.
	if err := lightstate.MirrorOnboarded(config.ResolveCloudBackupSettings(), path); err != nil {
		log.Printf("onboarding: cloud mirror failed after local write: %v", err)
	}
	return nil
}
