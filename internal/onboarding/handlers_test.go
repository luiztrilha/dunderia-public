...
func TestHandleCompleteKeepsSuccessWhenStateMirrorFails(t *testing.T) {
	withTempHome(t, func(_ string) {
		...
		if !s.Onboarded() {
			t.Fatal("expected onboarding state to persist locally despite mirror failure")
		}
	})
}
...
func onboardingTestAuthMiddleware(next http.HandlerFunc) http.HandlerFunc {
...
func TestRegisterRoutesRequiresAuthForMutations(t *testing.T) {
	withTempHome(t, func(_ string) {
		mux := http.NewServeMux()
		RegisterRoutes(mux, nil, "", onboardingTestAuthMiddleware)
		...
		cases := []struct {
			...
			{http.MethodPost, "/onboarding/progress", `{"step":"welcome","answers":{"company_name":"Initech"}}`},
			{http.MethodPost, "/onboarding/complete", `{"task":"Ship it","skip_task":true}`},
			{http.MethodPost, "/onboarding/checklist/dismiss", `{}`},
			{http.MethodPost, "/onboarding/checklist/pick_team/done", `{}`},
		}
		...
		for _, tc := range cases {
			...
			if w.Code != http.StatusUnauthorized {
				t.Fatalf("%s %s without auth: got %d, want %d (body: %s)", tc.method, tc.path, w.Code, http.StatusUnauthorized, w.Body.String())
			}
		}
		...
		openCases := []struct {
			...
			{http.MethodGet, "/onboarding/state", "", http.StatusOK},
			{http.MethodPost, "/onboarding/validate-key", `{"provider":"openai","key":"short"}`, http.StatusOK},
		}
		...
		for _, tc := range openCases {
			...
			mux.ServeHTTP(w, req)
			if w.Code != tc.want {
				t.Fatalf("%s %s without auth: got %d, want %d (body: %s)", tc.method, tc.path, w.Code, tc.want, w.Body.String())
			}
		}
	})
}

func TestRegisterRoutesAllowsAuthenticatedMutations(t *testing.T) {
	withTempHome(t, func(_ string) {
		mux := http.NewServeMux()
		RegisterRoutes(mux, nil, "", onboardingTestAuthMiddleware)
		...
		for _, tc := range cases {
			...
			mux.ServeHTTP(w, req)
			if w.Code != http.StatusOK {
				t.Fatalf("%s %s with auth: got %d, want %d (body: %s)", tc.method, tc.path, w.Code, http.StatusOK, w.Body.String())
			}
		}
	})
}
