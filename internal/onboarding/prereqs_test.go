...
func TestCheckAllReturnsFiveItems(t *testing.T) {
	results := CheckAll()
	if len(results) != 5 {
		t.Fatalf("CheckAll: got %d results, want 5", len(results))
	}
	names := []string{"node", "git", "claude", "codex", "gemini"}
...
func TestCheckOneGeminiConfiguredByEnv(t *testing.T) {
	t.Setenv("WUPHF_GEMINI_API_KEY", "test-gemini-key")

	r := CheckOne("gemini")
	if !r.Found {
		t.Fatal("expected gemini to be reported as ready when the API key is configured")
	}
	if !r.OK {
		t.Fatal("expected gemini OK=true when the API key is configured")
	}
	if r.Version == "" {
		t.Fatal("expected gemini readiness to expose a non-empty detail string")
	}
}
