param(
  [string]$Channel = 'convenios-legacy',
...
if ([string]::IsNullOrWhiteSpace($StatePath)) {
  $StatePath = Join-Path $teamDir 'convenios-legacy-watch-state.json'
}
if ([string]::IsNullOrWhiteSpace($LogPath)) {
  $LogPath = Join-Path $teamDir 'convenios-legacy-watch.jsonl'
}
if ([string]::IsNullOrWhiteSpace($SnapshotPath)) {
  $SnapshotPath = Join-Path $teamDir 'convenios-legacy-snapshot.json'
}
if (-not $PreferredWorkspaceRoots -or $PreferredWorkspaceRoots.Count -eq 0) {
  $PreferredWorkspaceRoots = @(
    'D:\Repos\ConveniosWebBNB_Antigo',
    'D:\Repositórios\ConveniosWebBNB_Antigo'
  )
}
