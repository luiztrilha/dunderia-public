...
- Runtime profile guidance lives in `docs/local-runtime-profile.md`. Treat `templates/starter-kit/` as the reusable public bootstrap and `dunderia-public-export/templates/local-runtime-profile` as reference-only snapshot material; never install or copy the broad snapshot wholesale.
- Living manual: keep `docs/MANUAL.md` current. Any change that affects user-visible behavior, runtime behavior, commands, configuration, integrations, UI, assets, security posture, recovery, or operational workflow must update the manual in the same change. When `docs/MANUAL.md` changes, regenerate `docs/MANUAL.pdf`. If no manual update is needed, state that explicitly in the handoff.
- After applying any new functionality in this repo, commit and push the change, then rebuild/restart the running local application so the online instance is not left on an older version. If validation or push/restart is blocked, report the exact blocker.
