...
function ToolCallCard({ item, compact }: { item: Record<string, unknown>; compact: boolean }) {
  ...
  const { summaryArg, summaryResult, summaryError } = useMemo(() => {
    ...
    let sumError = ''
    if (errorField != null) {
      sumError = typeof errorField === 'string' ? errorField.slice(0, 60) : t('messages.stream.error')
    }

    return { summaryArg: sumArg, summaryResult: sumResult, summaryError: sumError }
  }, [args, result, errorField, t])
      ...
      {open && (
        <div className="cc-tool-body">
          {Object.keys(cleanArgs).length > 0 && (
            <>
              <div className="cc-tool-section-label">{t('messages.stream.args')}</div>
              <Value value={cleanArgs} depth={1} compact={compact} />
            </>
          )}
          {result && Array.isArray(result.content) && result.content.length > 0 && (
            <>
              <div className="cc-tool-section-label cc-tool-result-label">{'\u2713 ' + t('messages.stream.response')}</div>
          ...
          {errorField != null && (
            <>
              <div className="cc-tool-section-label cc-tool-error">{'\u2717 ' + t('messages.stream.error')}</div>
...
function Collapsible({ label, startOpen, children }: { label: string; startOpen: boolean; children: ReactNode }) {
  ...
  if (open) {
    return (
      <span className="sv-collapsible">
        <button type="button" className="sv-toggle" onClick={() => setOpen(false)} title={t('messages.stream.collapse')}>
          ▾ {label}
        </button>
        {children}
      </span>
  ...
  return (
    <button type="button" className="sv-toggle" onClick={() => setOpen(true)} title={t('messages.stream.expand')}>
      ▸ {label}
    </button>
  )
}
