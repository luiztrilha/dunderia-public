...
async function forceFreshInstall(page: Page): Promise<void> {
...
async function advanceToTaskStep(page: Page): Promise<void> {
  ...
  await page.getByRole('button', { name: /continue|continuar/i }).click();
  await page.locator('input[type="password"]').first().fill('sk-onboarding-key');
  await page.getByRole('button', { name: /^ready$|^pronto$/i }).click();

  await page.locator('#wiz-task-input').fill('Stabilize onboarding UX');
}

async function advanceToSetupStep(page: Page): Promise<void> {
  await page.locator('.wizard-step button.btn-primary').first().click();
  await expect(page.locator('.template-grid')).toBeVisible({ timeout: 10_000 });
...
test.describe('wuphf onboarding wizard smoke', () => {
  ...
  test('failed onboarding completion keeps the wizard open', async ({ page }) => {
    await forceFreshInstall(page);
    ...
    await page.goto('/');
    await waitForReactMount(page);
    await advanceToTaskStep(page);

    await page.getByRole('button', { name: /start the first loop|iniciar o primeiro loop/i }).click();

    await expect(page.locator('.wizard-step').first()).toBeVisible({ timeout: 10_000 });
    await expect(page.getByText(/backend failed|falha|retry|tentar novamente/i).first()).toBeVisible({ timeout: 10_000 });
  });

  test('setup step can continue without entering a new api key', async ({ page }) => {
    await forceFreshInstall(page);
    await page.goto('/');
    await waitForReactMount(page);
    await advanceToSetupStep(page);

    const readyButton = page.getByRole('button', { name: /^ready$|^pronto$/i });
    await expect(readyButton).toBeEnabled({ timeout: 10_000 });
    await readyButton.click();

    await expect(page.locator('#wiz-task-input')).toBeVisible({ timeout: 10_000 });
  });
});
