...
function readPreferredEnv(...keys) {
...
function resolveSlug(input) {
    const slug = input ?? readPreferredEnv("WUPHF_AGENT_SLUG", "NEX_AGENT_SLUG");
    if (!slug) {
        throw new Error("Missing agent slug. Pass my_slug explicitly or set WUPHF_AGENT_SLUG (legacy NEX_AGENT_SLUG).");
    }
    return slug;
}
function authHeaders() {
    const token = readPreferredEnv("WUPHF_BROKER_TOKEN", "NEX_BROKER_TOKEN") || readBrokerTokenFile();
    if (!token)
        return {};
...
export function registerTeamTools(server) {
    server.tool("team_broadcast", "Post a message into the shared team channel so the human and every agent can see it.", {
        ...
        my_slug: z.string().optional().describe("Agent slug sending the message. Defaults to WUPHF_AGENT_SLUG (falls back to NEX_AGENT_SLUG)."),
    ...
    server.tool("team_poll", "Read recent messages from the shared team channel so you can stay in sync with teammates.", {
        my_slug: z.string().optional().describe("Your agent slug so tagged_count can be computed. Defaults to WUPHF_AGENT_SLUG (falls back to NEX_AGENT_SLUG)."),
        ...
        const slug = my_slug ?? readPreferredEnv("WUPHF_AGENT_SLUG", "NEX_AGENT_SLUG");
        const params = new URLSearchParams();
        if (slug)
            params.set("my_slug", slug);
        if (since_id)
    ...
    server.tool("team_status", "Share a short status update in the team channel. This is rendered as lightweight activity in the channel UI.", {
        ...
        my_slug: z.string().optional().describe("Agent slug sending the status. Defaults to WUPHF_AGENT_SLUG (falls back to NEX_AGENT_SLUG)."),
    ...
    server.tool("human_interview", "Ask the human a blocking interview question when the team cannot proceed responsibly without a decision. Presents options and optionally marks one as recommended. Pauses the team until answered.", {
        ...
        my_slug: z.string().optional().describe("Agent slug asking the question. Defaults to WUPHF_AGENT_SLUG (falls back to NEX_AGENT_SLUG)."),
