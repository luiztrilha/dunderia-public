...
- `~/.codex/skills/sql-exampleworkflow`: private database/repository skill tied to a non-public client environment.
...
`sql-exampleworkflow` is useful in the private environment but should not ship publicly because it names private repositories, database workflows, and operational tables. If a public user needs a SQL skill, create a new domain-neutral database skill that reads connection details from their own local config.
