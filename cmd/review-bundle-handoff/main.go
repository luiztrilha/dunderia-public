...
func cleanPathLikeInput(value string) string {
	if prefersSlashPaths(value) {
		return path.Clean(value)
	}
	return filepath.Clean(value)
}

func joinPathLikeInput(base, leaf string) string {
	if prefersSlashPaths(base) {
		return path.Join(path.Clean(base), leaf)
	}
	return filepath.Join(base, leaf)
}

func prefersSlashPaths(value string) bool {
