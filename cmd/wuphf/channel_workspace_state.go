...
func (s workspaceUIState) sidebarHintLine() string {
	...
	case strings.TrimSpace(s.Memory.NextStep) != "":
...
func (m channelModel) buildDirectIntroLines(contentWidth int) []renderedLine {
	...
	body := "Direct session reset. Agent pane reloaded in place. This surface is just you and the selected agent. No office channels and no colleague chatter. The door is closed."
...
func (m channelModel) buildOfficeFeedLines(contentWidth int) []renderedLine {
	...
	lines := buildOfficeMessageLines(m.messages, m.expandedThreads, contentWidth, m.threadsDefaultExpand, m.unreadAnchorID, m.unreadCount, m.executionNodeByMessageMap())
	lines = append(lines, buildLiveWorkLines(m.members, m.tasks, m.actions, contentWidth, "")...)
	return lines
}
