...
func (m channelModel) currentMainViewportLines(contentWidth, msgH int) []renderedLine {
	...
	executionNodeByMessage := m.executionNodeByMessageMap()
	bodyHeight := msgH
	...
	if m.activeApp == officeAppMessages {
		...
		lines := buildOfficeViewportSuffix(m.messages, m.expandedThreads, contentWidth, bodyHeight, m.scroll, m.threadsDefaultExpand, m.unreadAnchorID, m.unreadCount, m.members, m.tasks, m.actions, executionNodeByMessage)
		return append(needsYou, lines...)
	}
	return m.currentMainLines(contentWidth)
}

func buildOfficeViewportSuffix(messages []brokerMessage, expanded map[string]bool, contentWidth, msgH, scroll int, threadsDefaultExpand bool, unreadAnchorID string, unreadCount int, members []channelMember, tasks []channelTask, actions []channelAction, executionNodeByMessage map[string]brokerExecutionNode) []renderedLine {
	tail := buildLiveWorkLines(members, tasks, actions, contentWidth, "")
	return buildVirtualizedOfficeViewport(messages, expanded, contentWidth, msgH, scroll, threadsDefaultExpand, unreadAnchorID, unreadCount, executionNodeByMessage, tail)
}
...
func buildOneOnOneViewportSuffix(messages []brokerMessage, actions []channelAction, tasks []channelTask, members []channelMember, expanded map[string]bool, contentWidth, msgH, scroll int, agentName, agentSlug, unreadAnchorID string, unreadCount int) []renderedLine {
	...
	return buildVirtualizedOfficeViewport(messages, expanded, contentWidth, msgH, scroll, true, unreadAnchorID, unreadCount, nil, tail)
}

func officeThreadedMessages(messages []brokerMessage, expanded map[string]bool, threadsDefaultExpand bool, executionNodeByMessage map[string]brokerExecutionNode) []threadedMessage {
	return cachedThreadedMessages(messages, expanded, threadsDefaultExpand, executionNodeByMessage)
}

func renderOfficeMessageBlock(tm threadedMessage, contentWidth int, unreadAnchorID string, unreadCount int, executionNodeByMessage map[string]brokerExecutionNode) []renderedLine {
	msg := tm.Message
	...
	if strings.HasPrefix(msg.Kind, "human_") {
		...
		meta = appendExecutionContext(meta, msg.ID, executionNodeByMessage)
		if tm.Depth > 0 {
