...
func (m channelModel) cachedMainLines(contentWidth int) []renderedLine {
	...
	if m.isOneOnOne() {
		...
		case officeAppInbox:
			lines = buildInboxLines(filterMessagesForViewerScope(m.messages, m.oneOnOneAgentSlug(), "inbox"), m.requests, m.executionNodeByMessageMap(), contentWidth)
		case officeAppOutbox:
			lines = buildOutboxLines(filterMessagesForViewerScope(m.messages, m.oneOnOneAgentSlug(), "outbox"), m.actions, m.executionNodeByMessageMap(), contentWidth)
		default:
		...
		switch m.activeApp {
		case officeAppInbox:
			lines = buildInboxLines(m.messages, m.requests, m.executionNodeByMessageMap(), contentWidth)
		case officeAppOutbox:
			lines = buildOutboxLines(m.messages, m.actions, m.executionNodeByMessageMap(), contentWidth)
		case officeAppRecovery:
			lines = m.buildRecoveryLines(contentWidth)
		case officeAppTasks:
...
func (m channelModel) hashMainLinesState(contentWidth int) uint64 {
	...
	if m.isOneOnOne() || m.activeApp == officeAppMessages || m.activeApp == officeAppInbox || m.activeApp == officeAppOutbox || m.activeApp == officeAppRecovery {
		...
		h.addExecutionNodes(m.executionNodes)
		h.add(m.awaySummary)
...
func (s stateHasher) addExecutionNodes(nodes []brokerExecutionNode) {
	s.addInt(len(nodes))
	for _, node := range nodes {
		s.add(
			...
			node.OwnerAgent,
			node.Status,
			node.ExpectedResponseKind,
			node.ResolvedByMessageID,
			node.ResolvedByAgent,
			node.SupersedesNodeID,
			node.LastError,
		)
		s.add(strings.Join(node.ExpectedFrom, ","))
		s.addInt(node.AttemptCount)
		s.add(node.TimeoutAt)
		s.add(node.CreatedAt)
		s.add(node.UpdatedAt)
	}
}
