...
func TestChannelMsgFromOtherChannelIsIgnored(t *testing.T) {
	m := newChannelModel(false)
	m.activeChannel = "general"
	m.messages = []brokerMessage{
		{ID: "msg-1", From: "ceo", Content: "in general"},
	}
	m.lastID = "msg-1"

	next, _ := m.Update(channelMsg{
		requestChannel:  "launch",
		requestSequence: 1,
		messages: []brokerMessage{
			{ID: "msg-2", From: "pm", Content: "from launch"},
		},
	})
	got := next.(channelModel)
	if len(got.messages) != 1 || got.messages[0].ID != "msg-1" {
		t.Fatalf("expected launch messages to be ignored, got %+v", got.messages)
	}
}

func TestChannelMsgFromOutdatedPollSequenceIsIgnored(t *testing.T) {
	m := newChannelModel(false)
	m.activeChannel = "general"
	oldSeq := nextMessagePollSeq("general")

	next, _ := m.Update(channelMsg{
		requestChannel:  "general",
		requestSequence: oldSeq,
		messages: []brokerMessage{
			{ID: "msg-1", From: "ceo", Content: "first"},
		},
	})
	...
	_ = nextMessagePollSeq("general")
	next, _ = m.Update(channelMsg{
		requestChannel:  "general",
		requestSequence: oldSeq,
		messages: []brokerMessage{
			{ID: "msg-2", From: "pm", Content: "stale"},
		},
	})
	got := next.(channelModel)
	if len(got.messages) != 1 || got.messages[0].ID != "msg-1" {
		t.Fatalf("expected outdated poll sequence to be ignored, got %+v", got.messages)
	}
}

func TestResolvePendingResponsesClearedByReply(t *testing.T) {
	m := newChannelModel(false)
	m.activeChannel = "general"
	m.addPendingResponse("msg-1", "general", "msg-1")

	m.resolvePendingResponses(
		[]brokerMessage{
			{ID: "msg-2", From: "fe", ReplyTo: "msg-1", Content: "Done"},
		},
		nil,
	)

	if len(m.pendingResponses) != 0 {
		t.Fatalf("expected pending response to clear after reply, got %d", len(m.pendingResponses))
	}
}

func TestResolvePendingResponsesClearedByClosedExecutionNode(t *testing.T) {
	m := newChannelModel(false)
	m.activeChannel = "general"
	m.addPendingResponse("msg-3", "general", "msg-3")
	...
	if len(m.pendingResponses) != 0 {
		t.Fatalf("expected pending response to clear on closed execution node, got %d", len(m.pendingResponses))
	}
}

func TestResolvePendingResponsesClearedByResolvedByMessageID(t *testing.T) {
	m := newChannelModel(false)
	m.activeChannel = "general"
	m.addPendingResponse("msg-5", "general", "msg-5")
	...
	if len(m.pendingResponses) != 0 {
		t.Fatalf("expected pending response to clear when resolved by posted message id, got %d", len(m.pendingResponses))
	}
}

func TestResolvePendingResponsesPreservedForOpenExecutionNodeWithoutMatch(t *testing.T) {
	m := newChannelModel(false)
	m.activeChannel = "general"
	m.addPendingResponse("msg-7", "general", "msg-7")
	...
	if len(m.pendingResponses) != 1 {
		t.Fatalf("expected pending response to remain open, got %d", len(m.pendingResponses))
	}
}

func TestExpirePendingResponses(t *testing.T) {
	m := newChannelModel(false)
	now := time.Now()
	m.addPendingResponse("msg-9", "general", "msg-9")
	state := m.pendingResponses["msg-9"]
	state.StartedAt = now.Add(-(pendingResponseTimeout + 2*time.Second))
	m.pendingResponses["msg-9"] = state

	if expired := m.expirePendingResponses(now); expired != 1 {
		t.Fatalf("expected one expired response, got %d", expired)
	}
	if len(m.pendingResponses) != 0 {
		t.Fatalf("expected map to clear timed-out pending response, got %d", len(m.pendingResponses))
	}
}

func TestChannelTickMsgExpiresPendingResponses(t *testing.T) {
	m := newChannelModel(false)
	now := time.Now()
	m.addPendingResponse("msg-10", "general", "msg-10")
	state := m.pendingResponses["msg-10"]
	state.StartedAt = now.Add(-(pendingResponseTimeout + 2*time.Second))
	m.pendingResponses["msg-10"] = state

	next, _ := m.Update(channelTickMsg(now))
	got := next.(channelModel)
	if len(got.pendingResponses) != 0 {
		t.Fatalf("expected tick to clean expired pending response")
	}
	if got.notice != "Timed out waiting for 1 pending response(s)." {
		t.Fatalf("expected timeout notice, got %q", got.notice)
	}
}

func TestPostToChannelCapturesBrokerResponseID(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"id":"msg-11","duplicate":false,"total":7}`))
	}))
	t.Setenv("WUPHF_BROKER_BASE_URL", server.URL)
	t.Setenv("WUPHF_BROKER_TOKEN", "")

	msg := postToChannel("hello", "reply-1", "general")()
	got, ok := msg.(channelPostDoneMsg)
	if !ok {
		t.Fatalf("expected channelPostDoneMsg, got %T", msg)
	}
	if got.err != nil {
		t.Fatalf("expected no error, got %v", got.err)
	}
	if got.postedMessageID != "msg-11" {
		t.Fatalf("expected postedMessageID=msg-11, got %q", got.postedMessageID)
	}
	if got.postedReplyTo != "reply-1" {
		t.Fatalf("expected postedReplyTo=reply-1, got %q", got.postedReplyTo)
	}
	if got.postedChannel != "general" {
		t.Fatalf("expected postedChannel=general, got %q", got.postedChannel)
	}
	if got.duplicate {
		t.Fatalf("expected non-duplicate response")
	}
}

func TestPostToChannelPreservesDuplicateFlag(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"id":"msg-12","duplicate":true}`))
	}))
	t.Setenv("WUPHF_BROKER_BASE_URL", server.URL)
	t.Setenv("WUPHF_BROKER_TOKEN", "")

	msg := postToChannel("hello", "", "general")()
	got, ok := msg.(channelPostDoneMsg)
	if !ok {
		t.Fatalf("expected channelPostDoneMsg, got %T", msg)
	}
...
func TestNewChannelModelAutoStartsInitWithoutAPIKey(t *testing.T) {
	...
	if m.initFlow.IsActive() || m.initFlow.Phase() != tui.InitIdle {
		t.Fatalf("expected init flow to remain idle without external memory setup, got phase %q", m.initFlow.Phase())
	}
	if !strings.Contains(m.notice, "office-only mode") {
		t.Fatalf("expected local-only notice, got %q", m.notice)
	}
}

func TestNewChannelModelStaysLocalOnlyForLegacyMemorySelection(t *testing.T) {
	origHome := os.Getenv("HOME")
	t.Setenv("HOME", t.TempDir())
	...
	if m.initFlow.IsActive() || m.initFlow.Phase() != tui.InitIdle {
		t.Fatalf("expected init flow to stay idle with legacy memory selection, got phase %q", m.initFlow.Phase())
	}
	if !strings.Contains(m.notice, "office-only mode") {
		t.Fatalf("expected local-only notice, got %q", m.notice)
	}
...
func TestInitCommandRunsForLegacyMemoryBackend(t *testing.T) {
	origHome := os.Getenv("HOME")
	t.Setenv("HOME", t.TempDir())
	t.Setenv("WUPHF_MEMORY_BACKEND", "gbrain")
	t.Setenv("WUPHF_API_KEY", "")
	...
	if got.initFlow.Phase() != tui.InitAPIKey {
		t.Fatalf("expected legacy setup to ask for an API key first, got %q", got.initFlow.Phase())
	}
}
...
func TestChannelDoctorDoneShowsDoctorCard(t *testing.T) {
	...
	next, _ := m.Update(channelDoctorDoneMsg{report: channelDoctorReport{
		...
		Checks: []doctorCheck{{
			Label:    "Local memory",
			Severity: doctorWarn,
			Detail:   "Local memory is not configured.",
			NextStep: "Run /init and finish the local setup flow.",
		}},
	}})
	got := next.(channelModel)
	...
	if !strings.Contains(view, "Doctor") || !strings.Contains(view, "Local memory") {
...
func TestOfficeViewportWindowMatchesFullRenderAndMouseHitTesting(t *testing.T) {
	...
	full := append(buildOfficeMessageLines(m.messages, m.expandedThreads, contentWidth, m.threadsDefaultExpand, m.unreadAnchorID, m.unreadCount, nil), buildLiveWorkLines(m.members, m.tasks, m.actions, contentWidth, "")...)
...
func TestOfficeViewportVirtualizationCachesVisibleBlocks(t *testing.T) {
	...
	renderCalls := 0
	renderOfficeMessageBlockFn = func(tm threadedMessage, contentWidth int, unreadAnchorID string, unreadCount int, executionNodeByMessage map[string]brokerExecutionNode) []renderedLine {
		renderCalls++
		return renderOfficeMessageBlock(tm, contentWidth, unreadAnchorID, unreadCount, executionNodeByMessage)
	}
...
func TestMouseClickCollapsedThreadOpensThreadPanel(t *testing.T) {
	...
	headerH, msgH, _ := m.mainPanelGeometry(layout.MainW, layout.ContentH)
	contentWidth := layout.MainW - 2
	lines := buildOfficeMessageLines(m.messages, m.expandedThreads, contentWidth, m.threadsDefaultExpand, m.unreadAnchorID, m.unreadCount, nil)
...
func TestMergeExecutionNodesByID(t *testing.T) {
	m := newChannelModel(false)
	m.executionNodes = []brokerExecutionNode{
		{
			ID:            "exec-1",
			RootMessageID: "msg-a",
			Status:        "pending",
			ExpectedFrom:  []string{"agent-a"},
		},
		{
			ID:     "exec-2",
			Status: "done",
	...
	merged := m.mergeExecutionNodes([]brokerExecutionNode{
		{
			ID:            "exec-2",
			RootMessageID: "msg-b",
	...
	if len(merged) != 3 {
		t.Fatalf("expected 3 execution nodes after merge, got %d", len(merged))
	}
	if got := nodeByID(merged, "exec-2").RootMessageID; got != "msg-b" || gotStatus(merged, "exec-2") != "needs_correction" {
		t.Fatalf("expected exec-2 to be replaced, got root=%q status=%q", got, gotStatus(merged, "exec-2"))
	}
	if gotStatus(merged, "exec-3") != "timed_out" {
		t.Fatalf("expected exec-3 to be appended, got %q", gotStatus(merged, "exec-3"))
	}
}

func nodeByID(nodes []brokerExecutionNode, id string) brokerExecutionNode {
	...
	return brokerExecutionNode{}
}

func gotStatus(nodes []brokerExecutionNode, id string) string {
	return nodeByID(nodes, id).Status
}

func TestResetChannelFeedStateClearsExecutionNodes(t *testing.T) {
	m := newChannelModel(false)
	m.messages = []brokerMessage{{ID: "msg-1", From: "you", Content: "x"}}
	m.executionNodes = []brokerExecutionNode{{ID: "exec-1"}}

	m.resetChannelFeedState()
	if len(m.messages) != 0 || len(m.executionNodes) != 0 {
		t.Fatalf("expected feed state to clear messages and execution nodes, got messages=%d nodes=%d", len(m.messages), len(m.executionNodes))
	}
}
