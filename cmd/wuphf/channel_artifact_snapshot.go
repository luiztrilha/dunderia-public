...
func buildTaskRuntimeArtifact(task channelTask, logArtifact taskLogArtifact, hasLog bool) team.RuntimeArtifact {
	...
	return team.RuntimeArtifact{
		...
		UpdatedAt:     updatedAt,
		Path:          path,
		Worktree:      channelTaskWorkspacePath(task),
...
func buildTaskArtifactResumeHint(task channelTask, state string) string {
	if workspace := channelTaskWorkspacePath(task); workspace != "" {
		switch state {
		case "completed":
			return "Review the retained output or reopen the task thread before reusing the workspace."
		case "blocked":
			return "Resolve the blocker, then continue in " + workspace + " or reopen the task thread."
		default:
			return "Resume in " + workspace + " or reopen the task thread."
		}
	}
...
func channelTaskWorkspacePath(task channelTask) string {
