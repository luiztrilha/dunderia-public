...
func TestBuildOfficeIntroLinesUsesWorkspaceState(t *testing.T) {
	...
	if !strings.Contains(plain, "Welcome to the DunderIA office.") {
	...
	if !strings.Contains(plain, "Use channel history, scoped local memory, and cloud backup snapshots for recovery.") {
		t.Fatalf("expected local-only recovery guidance, got %q", plain)
	}
}
...
func TestBuildOfficeIntroLinesShowsOfflinePreviewGuidance(t *testing.T) {
	...
	if !strings.Contains(plain, "Launch DunderIA to attach the live office, or run /doctor to inspect runtime readiness.") {
