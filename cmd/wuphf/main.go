...
func printSubcommandHelp(sub string) {
	...
	case "init":
		...
		fmt.Fprintln(os.Stderr, "Runs first-time setup and saves your default provider")
		fmt.Fprintln(os.Stderr, "and startup choices so future `wuphf` invocations just work.")
		fmt.Fprintln(os.Stderr, "When cloud backup is configured, it also restores and re-syncs")
		fmt.Fprintln(os.Stderr, "the lightweight local machine state needed on a new PC.")
		fmt.Fprintln(os.Stderr, "")
	...
	case "shred", "kill":
		fmt.Fprintln(os.Stderr, "wuphf shred — stop the running office without deleting local state")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Legacy compatibility command. It stops the running session but preserves")
		fmt.Fprintln(os.Stderr, "channels, agents, messages, tasks, company identity, and workflows.")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Usage:")
	...
	case "repair-channel-memory":
		fmt.Fprintln(os.Stderr, "wuphf repair-channel-memory — rebuild channel memory from broker history")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Usage:")
...
func main() {
	...
	providerFlag := flag.String("provider", "", "LLM provider override for this run (claude-code, codex, gemini, gemini-vertex, ollama)")
	...
	noNex := flag.Bool("no-nex", false, "Legacy compatibility flag for older Nex-scoped configs (internal)")
	memoryBackend := flag.String("memory-backend", "", "Local memory backend for organizational context (none)")
	...
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr, "WUPHF v%s — local multi-agent office runtime.\n\n", buildinfo.Current().Version)
		fmt.Fprintf(os.Stderr, "Usage:\n")
		fmt.Fprintf(os.Stderr, "  %s              Launch multi-agent team (web UI on :%d)\n", appName, *webPort)
		fmt.Fprintf(os.Stderr, "  %s --tui        Launch with tmux TUI instead\n", appName)
		fmt.Fprintf(os.Stderr, "  %s init         Install the latest CLI, save setup defaults, and restore lightweight local state\n", appName)
	...
	if backend := strings.TrimSpace(*memoryBackend); backend != "" {
		...
		if normalized == "" {
			fmt.Fprintf(os.Stderr, "error: unsupported memory backend %q (expected none)\n", backend)
			os.Exit(1)
		}
	...
	if provider := strings.TrimSpace(*providerFlag); provider != "" {
		...
		default:
			fmt.Fprintf(os.Stderr, "error: unsupported provider %q (expected claude-code, codex, gemini, gemini-vertex, or ollama)\n", provider)
			os.Exit(1)
		}
	}
	...
	if len(args) > 0 {
		...
		case "shred", "kill":
			...
			fmt.Println("Office stop requested. Local channels, agents, messages, tasks, and workflows were preserved.")
			return
		case "init":
...
func runTeam(args []string, packSlug string, unsafe bool, oneOnOne bool, opusCEO bool, collabMode bool) {
	...
	if err := l.Attach(); err != nil {
		...
		fmt.Fprintf(os.Stderr, "Could not attach to tmux (not a terminal?). The office is running in the background.\n")
...
func dispatch(cmd string, apiKeyFlag string, format string) {
	...
	fmt.Fprintln(os.Stderr, "Non-interactive backend API commands are disabled in this fork. Use the local broker/web runtime instead.")
	os.Exit(1)
}
...
// want a broken config to block the user from stopping the office.
func killRunningSession(blueprint string) error {
	...
	if err != nil {
		...
		// Fall through silently; the state-preserving stop will still proceed.
		return nil
	}
	return l.Kill()
}
