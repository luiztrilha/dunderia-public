...
func buildInboxLines(messages []brokerMessage, requests []channelInterview, executionNodeByMessage map[string]brokerExecutionNode, contentWidth int) []renderedLine {
	...
	if len(messages) > 0 {
		...
		lines = append(lines, buildOfficeMessageLines(messages, map[string]bool{}, contentWidth, true, "", 0, executionNodeByMessage)...)
	}
	return lines
}

func buildOutboxLines(messages []brokerMessage, actions []channelAction, executionNodeByMessage map[string]brokerExecutionNode, contentWidth int) []renderedLine {
