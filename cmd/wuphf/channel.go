...
type channelMsg struct {
	...
	executionNodes  []brokerExecutionNode
...
func (m channelModel) executionNodeByMessageMap() map[string]brokerExecutionNode {
	return executionNodeByMessageIndex(m.executionNodes)
}

func (m channelModel) mergeExecutionNodes(incoming []brokerExecutionNode) []brokerExecutionNode {
	if len(incoming) == 0 {
		return cloneExecutionNodes(m.executionNodes)
	}
	if len(m.executionNodes) == 0 {
		return cloneExecutionNodes(incoming)
	}

	existing := cloneExecutionNodes(m.executionNodes)
	nodeByID := make(map[string]int, len(existing))
	for i, node := range existing {
		nodeByID[strings.TrimSpace(node.ID)] = i
	}
	...
	for _, node := range incoming {
		...
		if idx, ok := nodeByID[id]; ok {
			existing[idx] = cloneExecutionNode(node)
			continue
		}
		existing = append(existing, cloneExecutionNode(node))
		nodeByID[id] = len(existing) - 1
	}
	return existing
}

func cloneExecutionNode(node brokerExecutionNode) brokerExecutionNode {
	out := node
	if len(node.ExpectedFrom) > 0 {
		out.ExpectedFrom = append([]string(nil), node.ExpectedFrom...)
	}
	return out
}

func cloneExecutionNodes(nodes []brokerExecutionNode) []brokerExecutionNode {
	if len(nodes) == 0 {
		return nil
	}
	out := make([]brokerExecutionNode, len(nodes))
	for i, node := range nodes {
		out[i] = cloneExecutionNode(node)
	}
	return out
}

func (m *channelModel) resetChannelFeedState() {
	m.messages = nil
	m.executionNodes = nil
}

func (m *channelModel) addPendingResponse(postedMessageID, channel, expectedReplyTo string) {
	if m.pendingResponses == nil {
		m.pendingResponses = make(map[string]pendingResponseState)
	}

	postedMessageID = strings.TrimSpace(postedMessageID)
	if postedMessageID == "" {
		return
	}

	ch := normalizeSidebarSlug(channel)
	if ch == "" {
		ch = normalizeSidebarSlug(m.activeChannel)
		if ch == "" {
			ch = "general"
		}
	}

	replyTo := strings.TrimSpace(expectedReplyTo)
	if replyTo == "" {
		replyTo = postedMessageID
	}

	m.pendingResponses[postedMessageID] = pendingResponseState{
...
func (m *channelModel) executionNodeIsOpen(status string) bool {
	s := strings.ToLower(strings.TrimSpace(status))
	switch s {
	case "", "pending", "needs_correction", "blocked", "fallback_dispatched":
		return true
	default:
		return false
	}
...
func isNonHumanSender(from string) bool {
	sender := strings.ToLower(strings.TrimSpace(from))
	return sender != "" && sender != "you" && sender != "human" && sender != "nex"
}

func (m *channelModel) resolvePendingResponses(addedMessages []brokerMessage, executionNodes []brokerExecutionNode) {
	if len(m.pendingResponses) == 0 {
		return
	}

	repliedTo := make(map[string]struct{}, len(addedMessages))
	for _, msg := range addedMessages {
		if !isNonHumanSender(msg.From) {
			continue
		}
	...
	for postedID, state := range m.pendingResponses {
		if _, ok := repliedTo[state.ExpectedReplyTo]; ok {
			resolved = append(resolved, postedID)
			continue
		}
		for _, node := range executionNodes {
			if strings.TrimSpace(node.TriggerMessageID) != postedID && strings.TrimSpace(node.RootMessageID) != postedID {
				continue
			}
			if strings.TrimSpace(node.ResolvedByMessageID) != "" && strings.TrimSpace(node.ResolvedByMessageID) == postedID {
				resolved = append(resolved, postedID)
				break
			}
			if strings.TrimSpace(node.Status) != "" && !m.executionNodeIsOpen(node.Status) {
				resolved = append(resolved, postedID)
				break
			}
		}
...
func (m *channelModel) expirePendingResponses(now time.Time) int {
	if len(m.pendingResponses) == 0 {
		return 0
	}

	timedOut := make([]string, 0, len(m.pendingResponses))
	for postedID, state := range m.pendingResponses {
		if state.StartedAt.IsZero() {
			state.StartedAt = now
			m.pendingResponses[postedID] = state
			continue
		}
		if now.Sub(state.StartedAt) >= pendingResponseTimeout {
			timedOut = append(timedOut, postedID)
		}
	}

	for _, postedID := range timedOut {
		delete(m.pendingResponses, postedID)
	}
	return len(timedOut)
}

func (m channelModel) pendingResponseCount() int {
	if len(m.pendingResponses) == 0 {
		return 0
	}
	channel := normalizeSidebarSlug(m.activeChannel)
	count := 0
	for _, state := range m.pendingResponses {
		if normalizeSidebarSlug(state.Channel) == channel {
			count++
		}
	}
	return count
...
func (m channelModel) pendingResponseMeta() string {
	count := m.pendingResponseCount()
	if count == 0 {
		return ""
	}
	if count == 1 {
		return "Awaiting 1 pending agent response"
	}
	return fmt.Sprintf("Awaiting %d pending agent responses", count)
}

func executionNodeByMessageIndex(nodes []brokerExecutionNode) map[string]brokerExecutionNode {
	if len(nodes) == 0 {
		return map[string]brokerExecutionNode{}
	}

	mapping := make(map[string]brokerExecutionNode, len(nodes)*2)
	for i := len(nodes) - 1; i >= 0; i-- {
		node := nodes[i]
		for _, id := range []string{
			node.ResolvedByMessageID,
			node.TriggerMessageID,
			node.RootMessageID,
		} {
...
type brokerExecutionNode struct {
	ID                   string   `json:"id"`
...
const pendingResponseTimeout = 4 * time.Minute
...
var channelIntegrationSpecs = []channelIntegrationSpec{
	...
	{Label: "Google Calendar", Value: "google-calendar", Type: "calendar", Provider: "google", Description: "Connect Google Calendar and the office meeting bot"},
	{Label: "Outlook", Value: "outlook", Type: "email", Provider: "microsoft", Description: "Connect Microsoft email"},
	{Label: "Outlook Calendar", Value: "outlook-calendar", Type: "calendar", Provider: "microsoft", Description: "Connect Outlook Calendar and the office meeting bot"},
...
func newChannelModelWithApp(threadsCollapsed bool, initialApp officeApp) channelModel {
	...
	m := channelModel{
		...
		pendingResponses:     make(map[string]pendingResponseState),
	}
	if m.isOneOnOne() {
		m.sidebarCollapsed = true
		m.threadsDefaultExpand = true
		m.autocomplete = tui.NewAutocomplete(buildOneOnOneSlashCommands())
		m.notice = "Conference room reserved. Direct session reset. Agent pane reloaded in place."
	}
	memoryStatus := team.ResolveMemoryBackendStatus()
	if memoryStatus.SelectedKind == config.MemoryBackendNone {
		m.notice = "Running in office-only mode. Shared memory stays local."
	} else if strings.TrimSpace(memoryStatus.Detail) != "" {
		m.notice = "Legacy shared-memory config detected. Running in local-only mode."
	}
...
func (m channelModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "ctrl+g":
			if m.isOneOnOne() {
				m.setTransientNotice("1:1 mode: no sidebar, no distractions. Ideal.")
				return m, nil
			}
	...
	case channelPostDoneMsg:
		...
		if msg.err != nil {
			...
			m.notice = "Message already sent. Reusing existing message."
		...
		if msg.err == nil && !msg.duplicate && strings.TrimSpace(msg.postedMessageID) != "" {
			m.addPendingResponse(msg.postedMessageID, msg.postedChannel, msg.postedReplyTo)
		}
	...
	case channelMsg:
		requestedChannel := normalizeSidebarSlug(msg.requestChannel)
		if requestedChannel == "" {
			requestedChannel = normalizeSidebarSlug(m.activeChannel)
		}
		if requestedChannel != normalizeSidebarSlug(m.activeChannel) {
			break
		}
		if msg.requestSequence != 0 {
			if latest := latestMessagePollSeq(requestedChannel); latest != 0 && msg.requestSequence < latest {
				break
			}
		}

		m.executionNodes = m.mergeExecutionNodes(msg.executionNodes)
		hadHistory := m.lastID != ""
		addedMessages := make([]brokerMessage, 0)
		...
		m.resolvePendingResponses(addedMessages, m.executionNodes)
	...
	case channelTickMsg:
		m.tickFrame++
		if expired := m.expirePendingResponses(time.Now()); expired > 0 {
			m.notice = fmt.Sprintf("Timed out waiting for %d pending response(s).", expired)
		}
...
func (m channelModel) View() string {
	...
	if pendingMeta := m.pendingResponseMeta(); pendingMeta != "" {
		headerMeta += "  " + lipgloss.NewStyle().Foreground(lipgloss.Color("#F59E0B")).Render(pendingMeta)
	}
...
func postToChannel(text string, replyTo string, channel string) tea.Cmd {
	...
	return func() tea.Msg {
		...
		if resp.StatusCode < 200 || resp.StatusCode >= 300 {
			...
			if len(payload) > 0 {
				if err := json.Unmarshal(payload, &response); err == nil && strings.TrimSpace(response.Error) != "" {
					return channelPostDoneMsg{err: errors.New(strings.TrimSpace(response.Error))}
				}
				return channelPostDoneMsg{err: fmt.Errorf("%s", strings.TrimSpace(string(payload)))}
			}
			if len(payload) == 0 {
		...
		if len(payload) == 0 {
			return channelPostDoneMsg{err: errors.New("broker returned empty response")}
		}
		var response postMessageResponse
		if err := json.Unmarshal(payload, &response); err != nil {
			return channelPostDoneMsg{err: fmt.Errorf("failed to parse broker response: %w", err)}
		}
		if strings.TrimSpace(response.ID) == "" {
			return channelPostDoneMsg{err: errors.New("broker response missing message ID")}
		}
		return channelPostDoneMsg{
			postedMessageID: response.ID,
			postedChannel:   channel,
			postedReplyTo:   replyTo,
...
func (m channelModel) runCommand(trimmed, threadTarget string) (tea.Model, tea.Cmd) {
	...
	case trimmed == "/quit" || trimmed == "/exit" || trimmed == "/q":
		...
		return m, tea.Quit
	case trimmed == "/shred":
		// Legacy compatibility command. It is intentionally non-destructive.
		if _, err := workspace.Shred(); err != nil {
			m.notice = fmt.Sprintf("shred failed: %v", err)
			return m, nil
		}
		fmt.Fprintln(os.Stderr, "shred: local office state preserved; stopping session.")
		killTeamSession()
		return m, tea.Quit
	case trimmed == "/1o1":
		clearCurrent()
	...
	case trimmed == "/integrate":
		clearCurrent()
		m.notice = "Managed integrations are unavailable in local-only mode."
		return m, nil
	case trimmed == "/doctor":
		clearCurrent()
	...
	return func() tea.Msg {
		...
		if providerName == "codex" {
			...
			if err := l.ReconfigureSession(); err != nil {
				...
				if !strings.Contains(detail, "can't find pane") &&
					!strings.Contains(detail, "no server running") &&
					!strings.Contains(detail, `exec: "tmux"`) {
					return channelInitDoneMsg{err: err}
				}
			}
