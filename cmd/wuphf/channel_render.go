...
func buildOfficeMessageLines(messages []brokerMessage, expanded map[string]bool, contentWidth int, threadsDefaultExpand bool, unreadAnchorID string, unreadCount int, executionNodeByMessage map[string]brokerExecutionNode) []renderedLine {
	...
	if len(messages) == 0 {
		lines = append(lines,
			...
			renderedLine{Text: mutedStyle.Render("  The CEO triages first, then the right specialists pile in and ship.")},
		)
		return lines
	}

	lines = append(lines, renderedLine{Text: renderDateSeparator(contentWidth, "Today")})
	for _, tm := range officeThreadedMessages(messages, expanded, threadsDefaultExpand, executionNodeByMessage) {
		lines = append(lines, renderOfficeMessageBlock(tm, contentWidth, unreadAnchorID, unreadCount, executionNodeByMessage)...)
	}
...
func executionNodeForMessage(messageID string, executionNodeByMessage map[string]brokerExecutionNode) (brokerExecutionNode, bool) {
	if executionNodeByMessage == nil {
		return brokerExecutionNode{}, false
	}
	node, ok := executionNodeByMessage[strings.TrimSpace(messageID)]
	return node, ok
}

func appendExecutionContext(baseMeta, messageID string, executionNodeByMessage map[string]brokerExecutionNode) string {
	node, ok := executionNodeForMessage(messageID, executionNodeByMessage)
	if !ok || strings.TrimSpace(node.ID) == "" {
		return baseMeta
	}
	...
	if status := strings.TrimSpace(node.Status); status != "" {
		parts = append(parts, "status "+strings.ReplaceAll(status, "_", " "))
	}
	if kind := strings.TrimSpace(node.ExpectedResponseKind); kind != "" {
		parts = append(parts, "expected "+strings.ReplaceAll(kind, "_", " "))
	}
	if node.AttemptCount > 0 {
		parts = append(parts, "attempt "+strconv.Itoa(node.AttemptCount))
	}
	if root := strings.TrimSpace(node.RootMessageID); root != "" && root != messageID {
		parts = append(parts, "branch "+shortExecutionID(root))
	}
	if node.LastError != "" && (node.Status == "needs_correction" || node.Status == "timed_out") {
		parts = append(parts, "needs correction")
	}
	if len(parts) == 0 {
		return baseMeta
	}
	return baseMeta + " · " + strings.Join(parts, " · ")
}

func shortExecutionID(id string) string {
...
func buildOneOnOneMessageLines(messages []brokerMessage, expanded map[string]bool, contentWidth int, agentName string, unreadAnchorID string, unreadCount int) []renderedLine {
	if len(messages) == 0 {
		...
		return []renderedLine{
			...
			{Text: mutedStyle.Render("  No colleagues, no sidebar. Just you and " + agentName + ".")},
			{Text: ""},
			{Text: mutedStyle.Render("  Suggested: Help me think through the v1 launch plan.")},
			{Text: mutedStyle.Render("  Whatever you say here stays in this room until you bring it back to the office.")},
		}
	}
	return buildOfficeMessageLines(messages, expanded, contentWidth, true, unreadAnchorID, unreadCount, nil)
}
...
func buildPolicyLines(signals []channelSignal, decisions []channelDecision, alerts []channelWatchdog, actions []channelAction, contentWidth int) []renderedLine {
	...
	if len(signals) == 0 && len(decisions) == 0 && len(alerts) == 0 && len(actions) == 0 {
		...
		lines = append(lines, renderedLine{Text: ""})
		lines = append(lines, renderedLine{Text: muted.Render("  Use /policies to refresh this ledger.")})
		return lines
	}
...
func buildTaskLines(tasks []channelTask, contentWidth int) []renderedLine {
	...
	for _, task := range tasks {
		...
		if workspace := channelTaskWorkspacePath(task); workspace != "" {
			lines = append(lines, renderedLine{Text: "  " + muted.Render("Workspace: "+workspace), TaskID: task.ID})
		}
