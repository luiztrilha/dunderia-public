...
var (
	messagePollSeqMu         sync.Mutex
	messagePollSeq           uint64
	messagePollLatestPerChan = map[string]uint64{}
)

func nextMessagePollSeq(channel string) uint64 {
	channel = normalizeSidebarSlug(channel)
	if channel == "" {
		channel = "general"
	}
	messagePollSeqMu.Lock()
	defer messagePollSeqMu.Unlock()

	messagePollSeq++
	messagePollLatestPerChan[channel] = messagePollSeq
	return messagePollSeq
}

func latestMessagePollSeq(channel string) uint64 {
	channel = normalizeSidebarSlug(channel)
	if channel == "" {
		channel = "general"
	}
	messagePollSeqMu.Lock()
	defer messagePollSeqMu.Unlock()
	return messagePollLatestPerChan[channel]
}
...
func pollBroker(sinceID string, channel string) tea.Cmd {
	return func() tea.Msg {
		requestedChannel := normalizeSidebarSlug(channel)
		if requestedChannel == "" {
			requestedChannel = "general"
		}
		requestSeq := nextMessagePollSeq(requestedChannel)
		url := brokerURL("/messages?limit=100&channel=" + requestedChannel)
		if sinceID != "" {
		...
		var result struct {
			...
			ExecutionNodes []brokerExecutionNode `json:"execution_nodes"`
		}
		if err := json.Unmarshal(body, &result); err != nil {
			return channelMsg{}
		}
		if result.Channel != "" {
			requestedChannel = normalizeSidebarSlug(result.Channel)
		}
		if requestedChannel == "" {
			requestedChannel = "general"
		}
		return channelMsg{
			...
			requestChannel:  requestedChannel,
