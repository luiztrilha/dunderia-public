// DunderIA Pixel Office — scene engine
...
(function () {
  ...
  const ambientMessages = [
    ...
    { charId: 'jim',     text: "Unlike Ryan Howard's idea, this one works." },
  ...
  function drawWall() {
    ...
    // DunderIA sign (centered on viewport)
    ...
    ctx.fillStyle = C.yellow;
    ctx.font = `bold ${Math.round(sw / 10.5)}px "Press Start 2P"`;
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText('DUNDERIA', W / 2, sy + sh / 2);
    ctx.shadowBlur = 0;
  ...
  function drawMobileScene() {
    ...
    ctx.fillStyle = C.yellow;
    ctx.font = `bold ${Math.floor(sw / 9.5)}px "Press Start 2P"`;
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText('DUNDERIA', sx + sw / 2, sy + sh / 2);
    ctx.shadowBlur = 0;
    ctx.fillStyle = C.textMuted;
